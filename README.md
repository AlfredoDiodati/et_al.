...
├── random.h           # PCG64 RNG engine (uniform/normal/gamma/permutation) — explicit-state, reproducible streams; dist/ samplers build on it; see docs/RANDOM_DOCUMENTATION.md
├── lhs.h              # Latin hypercube sampling — space-filling designs over a box; above linalg/mat.h and random.h; see docs/LHS_DOCUMENTATION.md
...
`make test-npz-python` and `make test-lhs-r` are the two suites here that run another language's interpreter, and are deliberately outside `make test` and `check.sh` for it. `make test-lhs-r` checks `lhs.h` against R's `lhs` package; R is a development-tier dependency in exactly the standing numpy has here, and `docs/LHS_DOCUMENTATION.md` says what that suite can establish that a self-contained one cannot (two generators cannot produce the same designs, only the same distribution of designs).
...
As things stand, the core tier holds `linalg/mat.h`, `linalg/decomp.h`, `linalg/solver.h`, `ad.h`, `json.h`, `special.h`, `random.h`, `lhs.h`, `stats.h`, `inference/mcs.h`, `inference/unit_root.h`, `inference/cointegration.h`, `inference/qlr_test.h`, `dist/broadcast.h`, `dist/gauss.h`, `dist/student.h`, `dist/mv/gauss.h`, `dist/mv/student.h`, `dist/mv/matgauss.h`, `solver/optimizer.h`, `solver/adam.h`, `solver/lbfgs.h`, `frame/frame.h`, `frame/csv.h`, `frame/txt.h`, `frame/npy.h`, `frame/npz.h`, `frame/sql.h`, `frame/join.h`, `frame/rdata.h`, `frame/gzip.h`, and `cluster/cluster.h`; the model tier holds `nn/mlp.h`, `sd/qvarma.h` and `sd/score_driven_location.h`; and the development tier holds everything under `tests/`, `examples/`, and `scripts/`.
