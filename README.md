...
Clgebra is a pure C11 econometrics and machine learning compute stack, built to reach the performance class of JAX, NumPy, and numba without depending on a Python runtime. It combines a dense linear algebra core, general-purpose reverse-mode automatic differentiation, probability distributions, gradient-based optimizers, a `DataFrame` layer for loading, wrangling, and querying tabular data (including a small SQL engine), a JSON serializer for parameters and diagnostics, and neural network architectures, all built as a chain of layers on the same core and shipped as single-header C files. The one dependency the whole stack links against is OpenBLAS, for BLAS and LAPACK routines; everything else, from matrix arithmetic on up through model training and SQL query execution, is hand-written C with no further external libraries.

The dense linear algebra chain lives in `linalg/`: `mat.h` supplies the core `Mat` type, views over it, and element-wise/reduction arithmetic; `decomp.h` builds on `mat.h` for Cholesky, LU, QR, eigendecomposition, and SVD; `solver.h` builds on `decomp.h` for solving `Ax = b` and least squares. This chain sits in its own `linalg/` directory specifically so the bare name "solver" is free to mean something else higher up the stack — see [Adding files and headers](#adding-files-and-headers) for why that particular reuse is deliberate rather than an oversight.

Five further layers build on top of that core, and are independent of one another. `dist/` adds probability distributions, one file per distribution, each supplying a pdf, a log-pdf, and hand-derived log-pdf derivatives. `ad.h` adds general-purpose reverse-mode automatic differentiation: given any expression built from its operations, it computes the exact gradient with respect to any traced input, not tied to any specific loss or distribution, plus the `Activation`/`Criterion` function-pointer types a model built on top of it selects concrete operations through. `solver/` adds gradient-based optimizers behind a generic pluggable `Optimizer` interface that updates a parameter given any gradient, however it was produced — a hand-derived one from `dist/`, a tape-computed one from `ad.h`, or anything else. `frame/` adds `DataFrame`, a matrix plus optional row and column labels and typed numeric-or-string columns, needing only `linalg/mat.h`; loaders for CSV, whitespace-delimited text, and NumPy's `.npy` format sit above it, and so does a small SQL query engine (`SELECT`/`FROM`/`WHERE`/`GROUP BY`/`ORDER BY`) that executes real SQL text directly against a `DataFrame`. `json.h` adds a JSON value tree for parsing, building, and writing hyperparameters, fit diagnostics, and configuration — the one layer genuinely outside the core chain, with zero dependency on `linalg/mat.h` at all, since a parameter is typically a standalone scalar rather than part of a matrix computation.
...
Each header has its own documentation file under `docs/`, covering its full API reference, behavioral contracts, and known limitations: `MATRIX_DOCUMENTATION.md` for `linalg/mat.h`, `DECOMP_DOCUMENTATION.md` for `linalg/decomp.h`, `SOLVER_DOCUMENTATION.md` for `linalg/solver.h`, `GAUSS_DOCUMENTATION.md` for `dist/gauss.h`, `AD_DOCUMENTATION.md` for `ad.h`, `OPTIMIZER_DOCUMENTATION.md` for `solver/optimizer.h`, `ADAM_DOCUMENTATION.md` for `solver/adam.h`, `FRAME_DOCUMENTATION.md` for `frame/frame.h`, `CSV_DOCUMENTATION.md` for `frame/csv.h`, `TXT_DOCUMENTATION.md` for `frame/txt.h`, `NPY_DOCUMENTATION.md` for `frame/npy.h`, `SQL_DOCUMENTATION.md` for `frame/sql.h`, `JSON_DOCUMENTATION.md` for `json.h`, and `MLP_DOCUMENTATION.md` for `nn/mlp.h`. This file, by contrast, covers directory structure, build instructions, installation, the policies that govern how the codebase grows, the design principles behind the dense linear algebra core, and a set of pitfalls that apply across the whole project — no API documentation lives here. What follows is organized in that order: directory structure, build, installation, testing and benchmarking, policies, design principles, and pitfalls.
...
As things stand, the core tier holds `linalg/mat.h`, `linalg/decomp.h`, `linalg/solver.h`, `ad.h`, `json.h`, `dist/gauss.h`, `solver/optimizer.h`, `solver/adam.h`, `frame/frame.h`, `frame/csv.h`, `frame/txt.h`, `frame/npy.h`, and `frame/sql.h`; the model tier holds `nn/mlp.h`; and the development tier holds everything under `tests/`, `examples/`, and `scripts/`.
