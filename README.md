...
As things stand, the core tier holds `linalg/mat.h`, `linalg/decomp.h`, `linalg/solver.h`, `ad.h`, `json.h`, `special.h`, `random.h`, `stats.h`, `dist/broadcast.h`, `dist/gauss.h`, `dist/student.h`, `dist/mv/gauss.h`, `dist/mv/student.h`, `solver/optimizer.h`, `solver/adam.h`, `frame/frame.h`, `frame/csv.h`, `frame/txt.h`, `frame/npy.h`, and `frame/sql.h`; the model tier holds `nn/mlp.h`; and the development tier holds everything under `tests/`, `examples/`, and `scripts/`.
...
**Do not let a dependency in through a build that happens to provide it.** The only dependencies this project allows are **OpenBLAS and whatever ships with GCC**. Anything else is a bug even when it compiles on your machine, because "it compiles" is a fact about one machine's installed packages, not about the code. This has happened twice here, and both times the code was written where the extra piece happened to be present and nobody noticed.

*LAPACKE* was the first and the expensive one. OpenBLAS exports the raw Fortran LAPACK symbols but not a single `LAPACKE_` one - `nm -D /lib/x86_64-linux-gnu/libopenblas.so.0 | grep -c LAPACKE_` prints `0`, and `dpkg -S /usr/include/lapacke.h` names a separate package, `liblapacke-dev`. Nothing in the Makefile ever asked for it, so a second undeclared dependency rode along until the build failed to link on the first machine without that package. Unpicking it meant reimplementing seventeen LAPACK routines against CBLAS - see `docs/FACTOR_DOCUMENTATION.md`.

*OpenMP* was the second, and it shows the subtler shape: it arrived not from a separate package but from a **different build of a dependency already allowed**. `pkg-config --cflags openblas` hands back `-fopenmp` when OpenBLAS was itself built against OpenMP, and nothing when it was built against pthreads. Debian ships both under the same name. `frame/sql.h` had guarded this correctly and stubbed a fallback, but only for the one OpenMP entry point it called at the time; the multi-threaded `GROUP BY` added two more later and the fallback was never extended, so the build stopped with `undefined reference to omp_get_thread_num` on a pthreads machine.
