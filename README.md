...
A further rule binds the cache these models write. `<model>_fit_cached` reloads a fit the solver finished with, and **resumes one that stopped at its iteration cap** rather than returning it: a capped fit is a starting point, not an answer, and handing it back would mean a script could be rerun for ever without the estimate moving. Only the cap is resumed from, since a run that stalled or went non-finite did not run short of iterations and repeating it costs a whole fit for nothing — which is why the reason the search stopped is stored rather than derived from the convergence flag on load. What a chain of runs cost is then `total_niter`, the iterations summed over every run, beside `nruns`, how many runs there were; `niter` stays this run alone. A cache is also a file a user can edit, so a field missing from it is a refusal to load rather than an assert, and a refusal leaves the caller's model untouched.
