...
│   └── tensor.h                    # n-dimensional array over the same row-major buffers Mat uses — views, NumPy broadcasting, reductions over any axis set, batched matmul, tensordot, einsum; a Mat is its rank-2 case and converts to and from it with no copy; threaded under -fopenmp; see docs/TENSOR_DOCUMENTATION.md and docs/TENSOR_PERFORMANCE_DOCUMENTATION.md
│
├── ad.h               # reverse-mode autodiff (backprop) — general-purpose; includes linalg/solver.h + linalg/tensor.h + special.h (ad_lgamma, ad_lgamma_diff). Differentiates Mat expressions and, through TensorNode, n-dimensional ones including batched matmul and einsum; see docs/AD_DOCUMENTATION.md and docs/AD_TENSOR_DOCUMENTATION.md
...
│
├── random/                         # randomization: the bit generator, and the sampling schemes built on it
│   ├── random.h                    # PCG64 RNG engine (uniform/normal/gamma/permutation) — explicit-state, reproducible streams, no dependency on linalg/mat.h; dist/ samplers build on it; see docs/RANDOM_DOCUMENTATION.md
│   └── lhs.h                       # Latin hypercube sampling — space-filling designs over a box; above linalg/mat.h and random/random.h; see docs/LHS_DOCUMENTATION.md
│
├── inference/                      # hypothesis tests on data somebody already has — one file per family, above stats.h; each returns a statistic and a verdict, never a fitted model
│   ├── unit_root.h                 # unit root tests on one series (ADF, KPSS, DF-GLS, Otto) plus the break tests (Zivot-Andrews, HLT, HHLT) — above linalg/solver.h/random/random.h/stats.h; see docs/UNIT_ROOT_DOCUMENTATION.md and docs/BREAK_TESTS_DOCUMENTATION.md
...
│   └── mcs.h                       # tests of equal predictive ability: model confidence set + Diebold-Mariano — above stats.h/random/random.h/special.h/frame/frame.h; see docs/MCS_DOCUMENTATION.md, docs/MCS_PERFORMANCE_DOCUMENTATION.md, docs/MCS_RELIABILITY_DOCUMENTATION.md and docs/MCS_TESTING_DOCUMENTATION.md
│
...
│
├── frame/                          # DataFrame: data loading/wrangling/querying — above linalg/mat.h. npy.h also reads and writes n-dimensional arrays as linalg/tensor.h Tensors, which is how a stack of matrices crosses to and from numpy
│   └── gzip.h                      # gzip container + from-scratch DEFLATE decoder and encoder — no dependency on linalg/mat.h or frame.h; exists because zlib is not OpenBLAS/GCC (see Dependencies below); the decoder for rdata.h, the encoder for npz.h; see docs/GZIP_DOCUMENTATION.md
│
...
│   │
│   ├── integration/                # does the hand-off between two modules hold? — one file per seam, make test-integration; see docs/INTEGRATION_TESTS_DOCUMENTATION.md and the two seam files it indexes
│   │
...
make examples/tensor_example    # matrices stacked over time: views, batched products,
./examples/tensor_example       # einsum, reductions, and a .npy round trip to and from numpy
...
make examples/lhs_example       # a Latin hypercube parameter design, and what it buys
./examples/lhs_example          # writes examples/out/lhs_example_design.csv and its report
...
make test-npz-python        # frame/npz.h against a live numpy; not part of `make test`
make test-lhs-r             # lhs.h against a live R and its lhs package; not part of `make test`
...
`make test-npz-python` and `make test-lhs-r` are the two suites here that run another language's interpreter, and are deliberately outside `make test` and `check.sh` for it. `make test-lhs-r` checks `random/lhs.h` against R's `lhs` package; R is a development-tier dependency in exactly the standing numpy has here, and `docs/LHS_DOCUMENTATION.md` says what that suite can establish that a self-contained one cannot (two generators cannot produce the same designs, only the same distribution of designs).

On the Python half: numpy and pandas are development-tier dependencies, so the shipped suite has to build and pass with OpenBLAS and a C compiler alone; what that costs is that nothing in it can call `np.load`, which is the only way to check a *writer* against real numpy rather than against this project's own reader. `tests/correctness/test_npz.c` closes as much of that gap as a Python-free suite can, by embedding archives numpy produced once during development. See `docs/NPZ_DOCUMENTATION.md`.

The statistical test suites (`inference/unit_root.h`, `inference/cointegration.h`, `inference/qlr_test.h`, `solver/lbfgs.h`, `sd/score_driven_location.h`) are built at float64 **whatever `MAT_DOUBLE` says**, through the Makefile's `STAT_CFLAGS` rather than `CFLAGS`. That is not a preference: the regressions underneath a unit root or co-integration statistic are ill-conditioned by construction, a levels regressor against its own difference, and in float32 the published critical values they are checked against are not reproduced to the digits the papers print.

`sd/qvarma.h` is built one script at a time instead, through `MODEL_CFLAGS` (float32) or `STAT_CFLAGS` (float64), because a fit and the standard errors at that fit have different precision requirements and most scripts only want the fit. The model reports what it could not compute at the precision it was built at rather than aborting on it — `qvarma_standard_errors` returns `hessian_is_usable == 0` where it used to die inside `mat_eig_sym`. What the choice costs and saves is measured in `docs/QVARMA_DOCUMENTATION.md`'s Building section; the eigensolver side of it is `mat_eig_sym_status` in `docs/DECOMP_DOCUMENTATION.md`.
...
`check.sh` runs every test suite, writes the full output to `test_report.txt`, and prints a one-line PASS/FAIL summary per suite to the terminal. It is wired as a git pre-commit hook, so `git commit` will refuse if any suite fails. To install the hook in a fresh clone, run `bash scripts/install-hooks.sh`. `test_report.txt` is generated output and is committed, not ignored: it is the record of what the suite last did, in the same way `bench_report.txt` is the record of what the benchmarks last measured, and `docs/PERFORMANCE_BACKLOG.md` cites the latter as the source of its numbers. A commit therefore carries the test run that justified it.
...
python tests/performance/bench_npz.py      # frame/npz.h + frame/gzip.h vs numpy.savez/zlib
python tests/performance/bench_random.py   # random/random.h vs numpy.random.Generator
python tests/performance/bench_stats.py    # stats.h vs NumPy
make bench-lhs                             # lhs.h vs R's lhs package (R, not Python)
```
...
The examples are built by `make examples`, which `check.sh` runs alongside the test binaries. An example is documentation that has to keep compiling, and until that target existed nothing built any of them: each had its own rule and no target named them together.

Not every binary under `tests/` is in `make test`. `tests/correctness/qvarma_recovery_study.c` is a Monte Carlo study rather than a pass/fail check — it writes `out/qvarma_recovery_study.txt` and prints nothing — and has its own `make study-qvarma_recovery` target, since it fits hundreds of models. The `tests/performance/lbfgs_*` files compare candidate implementations of `solver/lbfgs.h` against the ones they would replace and are run directly, like the other standalone design-space benchmarks listed in the Makefile: `make bench-small_blas_threshold` for the size at which a BLAS call stops paying for itself, which is where `MAT_GEMM_SMALL` and `TRSM_SMALL_N` come from, `make bench-tensor_omp_threshold` for the element count above which threading each kernel pays, which is where `linalg/tensor.h`'s four `TENSOR_OMP_MIN_*` constants come from, `make bench-tensor_reduce_tile` for whether tiling a reduction over an outer axis pays, which it does in isolation and does not once it is inside the macro every reduction expands, so that one benchmarks something deliberately absent from the header, `make bench-tensor_batch_threads` for the band of matrix sizes in which threading a batch of products pays, bounded above because past it OpenBLAS threads a single product itself and the two parallelisms multiply into a fiftyfold loss, and `make bench-qvarma_analytic_filter` for the analytic-gradient evaluator in `sd/qvarma.h` — the one whose gradient is derived by hand rather than taken from the tape — against the taped one, at one thread and at every hardware thread the machine has, `make bench-qvarma_cluster_fits` for a batch of fits running with both levels of parallelism at once, an OpenMP loop over the fits inside a machine and `cluster/cluster.h` between machines, and `make bench-qvarma_batched_filter` for whether evaluating several series in one vectorized loop would beat evaluating them one at a time, which it does not by enough to be worth building, and `make bench-mcs_candidates MCS_CANDIDATE=<header>` for a candidate rewrite of `inference/mcs.h` against the shipped one, which compares them on agreement before cost and exits nonzero when they disagree — that one builds the two versions into two translation units of one binary, since a header of `static inline` functions cannot be included twice but can be compiled twice and linked once.

**`tests/integration/` answers a third question: does the hand-off between two modules hold.** Every suite under `tests/correctness/` tests one header, and each one is right to. The consequence is that a module can be correct on its own, its neighbour can be correct on its own, and the composition of the two can be wrong with nothing failing anywhere — which is not a hypothetical here: at the time the directory was added, no test in this repository loaded a file, built a `DataFrame` from it, and handed the result to a statistic or a model, which is the only way this library is ever actually used. Those checks live in `tests/integration/`, run with `make test-integration`, and `check.sh` runs them after the correctness suites. See `docs/INTEGRATION_TESTS_DOCUMENTATION.md` for the directory's rules and an index of every file, and `docs/INTEGRATION_DATA_SEAMS_DOCUMENTATION.md`/`docs/INTEGRATION_STRUCTURE_SEAMS_DOCUMENTATION.md` for what each one covers and what it found.

The test for whether a check belongs there rather than in `tests/correctness/` is mechanical: **an integration test includes headers from at least two different directories, and its subject is the seam between them, not either side of it.** A test that includes `stats.h` to generate a series for a `linalg/` check is still a correctness test — the second header is scaffolding. A test that asserts a `frame/` column and an `inference/unit_root.h` statistic agree about what the data is, is not. Where a module's own contract is what is under test, it belongs in `tests/correctness/` even if the file happens to include something else to set the input up.
...
A **third group arrives with `-fopenmp`**, which the default build now passes (see below) — 4 entry points, resolved by `libgomp.so.1`:

```
4 OpenMP      GOMP_parallel omp_get_max_threads omp_get_num_threads
              omp_get_thread_num
```

Checked the same way rather than assumed: `nm --undefined-only` on the object that references every public entry point of every header. `linalg/tensor.h` accounts for three of them and `frame/sql.h` for the fourth; `linalg/tensor.h` calls no OpenMP function itself, so its three come from `#pragma omp` alone. Dropping the flag (`make OPENMP_CFLAGS=`) removes all four and the whole project still builds and passes — this is the one group that is a property of the build rather than of the code. `libgomp` ships with GCC, which puts it in the same half of the policy as the POSIX group above and unlike OpenBLAS makes it nothing a user has to install.
...
**The default build now passes `-fopenmp`** (`OPENMP_CFLAGS` in the Makefile, and the flag is written into the installed `et_al.-core.pc`), because `linalg/tensor.h`'s element-wise kernels, reductions and batched product each have a parallel form that is simply not reached without it — that header tests the predefined `_OPENMP` macro and takes a different, serial walk when the compiler was not told to thread. This is a speed flag and not a behavioural one: every path it guards computes the same answers either way, and `make test` builds and runs the whole tensor correctness suite both ways, as `test_tensor` and `test_tensor_serial`. `make OPENMP_CFLAGS= test` builds the entire project without it and still passes, on one thread. Nothing in `linalg/tensor.h` calls an OpenMP entry point — only `#pragma omp` and `_OPENMP` — so unlike `frame/sql.h` it has nothing to stub.

The same reasoning ruled out zlib for `frame/rdata.h` (reads R's `.RData`/`.rds` files, which `save()` gzip-compresses by default) before it could repeat the LAPACKE story: zlib is neither OpenBLAS nor something GCC ships, it is a separate package (`libz-dev`) that happens to be installed on most machines, indistinguishable from the LAPACKE situation until a build on a machine without it fails to link. `frame/gzip.h` reimplements gzip's container framing and DEFLATE compression and decompression directly against libc instead, the same "write the small missing piece rather than pull in a library for it" call `linalg/factor.h` made for LAPACKE. See `docs/GZIP_DOCUMENTATION.md`.
...
A documentation file must be scannable in a single pass: a minimum of roughly 50 lines, since shorter content does not justify its own file and belongs in the most relevant existing one instead, and a maximum of roughly 300 lines, above which the file becomes expensive to navigate and the largest self-contained section should be extracted into a new, clearly-named file linked back from the original. Four headers currently have more than one file each on exactly that ground: `inference/unit_root.h`, whose break tests are in `docs/BREAK_TESTS_DOCUMENTATION.md`; `sd/qvarma.h`, whose measured reliability limits are in `docs/QVARMA_RELIABILITY_DOCUMENTATION.md`; `linalg/tensor.h`, whose measured constants, threading decisions and benchmark table are in `docs/TENSOR_PERFORMANCE_DOCUMENTATION.md`, read when changing a kernel rather than when writing a call; and `inference/mcs.h`, which has four, split by which of four questions a reader arrives with — how do I call it (`docs/MCS_DOCUMENTATION.md`), how fast is it and how do I change it (`docs/MCS_PERFORMANCE_DOCUMENTATION.md`), can I believe the answer at my sample size (`docs/MCS_RELIABILITY_DOCUMENTATION.md`), and what does each suite hold in place (`docs/MCS_TESTING_DOCUMENTATION.md`). `tests/integration/` is split three ways for the same reason: `docs/INTEGRATION_TESTS_DOCUMENTATION.md` holds the directory's scope rule, its two specific rules, how it is built and run, an index of every file, and what it has already changed in the library; `docs/INTEGRATION_DATA_SEAMS_DOCUMENTATION.md` holds the files whose subject is a value crossing between modules, read when data looks wrong; `docs/INTEGRATION_STRUCTURE_SEAMS_DOCUMENTATION.md` holds the files whose subject is the library's own structure, read before changing an interface, adding a header or distributing a run. All of these extractions split by *when a reader needs the content* rather than by size alone — the break tests answer a question the other three do not, the reliability limits are what a reader consults after a fit has already run rather than while writing the call, and the three MCS files are what a reader opens before changing that header, before trusting a set it returned, or when one of its suites fails, rather than before calling it. Topics that span multiple layers, such as memory ownership, special-value behavior, or row-major layout, go in the documentation file of the lowest layer where they first become relevant, not in a separate file — `MATRIX_DOCUMENTATION.md` is currently the right home for every cross-cutting topic of that kind, since every other layer builds on `linalg/mat.h`.

### Adding files and headers

Create a new `.h` file only when a group of functions introduces a concept that does not belong in the current lowest layer. The test is the include direction: if the new functions call existing ones but existing ones never need to call them back, a new header is warranted. A new layer must fit into the `linalg/mat.h <- linalg/decomp.h <- linalg/solver.h` chain, extend it downward (below `linalg/mat.h`, closer to hardware), or extend it upward (above the topmost existing layer, further from hardware). Fifteen layers currently sit above (or, for `json.h`, `special.h`, and `random/random.h`, entirely outside) the core this way — `dist/`, `ad.h`, `json.h`, `special.h`, `random/`, `stats.h`, `inference/mcs.h`, `inference/unit_root.h`, `inference/cointegration.h`, `inference/qlr_test.h`, `solver/`, `frame/`, `nn/`, `sd/` — each for its own new concern: distributions, general differentiation, JSON serialization, scalar special functions, pseudo-random number generation, space-filling sampling designs, sample statistics of observed data, tests of equal predictive ability between competing forecasts, unit root and structural break tests on one series, co-integration tests between series, testing for the absence of score-driven dynamics, gradient-based optimization, data loading/wrangling/querying, network architectures, and score-driven time-series models. These are mostly independent of each other, with a few exceptions to the flat picture: `dist/` draws on `special.h` and `random/random.h` (`dist/student.h`'s digamma-based `nu` score; the `*_sample` functions), `ad.h` draws on `special.h` (`ad_lgamma`'s digamma backward rule), `stats.h` draws sideways on `special.h` (the chi-squared tail `stats_ljung_box`'s p-value needs), `inference/mcs.h` draws on `stats.h`, `random/random.h`, `special.h` and `frame/frame.h` at once (its HAC variance, its bootstrap draws, its normal tail and its labelled input), `random/lhs.h` draws on `random/random.h` and `linalg/mat.h` (a design is a permutation plus jitter, returned as a `Mat`) and is not a `dist/` file, since its rows are dependent on each other by construction rather than being independent draws from a law, `inference/cointegration.h` is the one of these built directly on another of them (`inference/unit_root.h`, since Engle-Granger's second step is an ADF regression), and `nn/`/`sd/` build directly on `ad.h` rather than the core — a valid "extend upward" move all the same, just from a point higher up the chain than `linalg/solver.h`.

`inference/unit_root.h`, `inference/cointegration.h`, `inference/qlr_test.h` and `inference/mcs.h` are four headers rather than one, because the include direction separates them: `cointegration.h` calls into `unit_root.h` and `unit_root.h` never needs to call back, `qlr_test.h` calls into neither, and `mcs.h` sits over `frame/frame.h` instead. Merging them would put a co-integration concern inside the file a caller includes to run an ADF. They share `inference/` because they answer the same kind of question — a statistic and a verdict about data somebody already has, with no fitted model coming back — and because four of them at the repository root left nothing to distinguish a layer of the core chain from a test built on top of it. `ljung_box` is the counter-example that shows where the line is — it is one function over `stats_autocorr`, so it went into `stats.h` rather than getting a fifth file in `inference/`. Do not add a header at the same level as an existing one that duplicates its role — merge into it instead, which is why eigendecomposition and SVD live in `linalg/decomp.h` rather than a new file, since they're the same conceptual role (decomposition) as Cholesky/LU/QR. `dist/`/`solver/`/`frame/`/`nn/` are new directories for the same reason a new layer sometimes needs one: each covers a wholly new concern that doesn't belong inside `tests/`, `examples/`, or the existing chain's files. `json.h`, by contrast, is one cohesive concept rather than a family of interchangeable files the way `dist/`'s distributions or `solver/`'s algorithms are, so it stays a single root file rather than getting its own directory, the same reasoning `ad.h` already follows.
...
Naming follows the layer a file belongs to. A member of the dense linalg chain is `linalg/<noun>.h`, matching `mat.h`, `tensor.h`, `decomp.h`, and `solver.h`. A standalone core layer with no directory of its own is `<noun>.h` at the repository root, matching `ad.h`, `json.h`, `special.h`, and `stats.h`. A randomization concern is `random/<noun>.h`, matching `random.h` (the bit generator and its scalar variates) and `lhs.h` (Latin hypercube designs). `random/random.h` repeats its directory's name the way `frame/frame.h` and `cluster/cluster.h` do, for the same reason: it is the base file of the group, the one every other member includes. A hypothesis test is `inference/<noun>.h`, matching `unit_root.h`, `cointegration.h`, `qlr_test.h`, and `mcs.h`. A distribution is `dist/<noun>.h`, matching `gauss.h`; a multivariate distribution — one whose density couples the components of a vector-valued observation and therefore needs `linalg/decomp.h`'s factorizations, not just element-wise formulas — is `dist/mv/<noun>.h`, matching `mv/gauss.h`, reusing the univariate file's noun when it is the same family. A matrix-variate distribution, whose observation is a whole matrix and which therefore carries one covariance per axis, lives in the same directory with the noun prefixed by `mat`, matching `mv/matgauss.h` — the same layer and the same factorization dependency, so a fourth directory would say nothing the prefix does not. An optimizer is `solver/<noun>.h`, matching `adam.h`. A data loading, wrangling, or querying concern is `frame/<noun>.h`, matching `frame.h`, `csv.h`, `txt.h`, `npy.h`, `npz.h`, `sql.h`, `join.h`, `rdata.h`, and `gzip.h`. `gzip.h` is the one member of that directory that does not include `frame.h` and knows nothing about a `DataFrame`: it is compression, not data loading. It lives here because both of its callers do — `rdata.h` needs the decoder for R's gzip-compressed `save()` output, `npz.h` needs the encoder for a compressed zip member — and nothing outside `frame/` has ever needed it. A future caller elsewhere in the tree would be the reason to move it back to the root, not a reason it is misplaced now. A distributed-execution concern is `cluster/<noun>.h`, matching `cluster.h`. A network architecture is `nn/<noun>.h`, matching `mlp.h`; a score-driven time-series model is `sd/<noun>.h`, matching `qvarma.h` and `score_driven_location.h`. A correctness test is `tests/correctness/test_<noun>.c`, a benchmark is a `tests/performance/bench_<noun>.c` and `bench_<noun>.py` pair, and a usage example is `examples/<noun>_example.c`.
...
Two rules bind this shape specifically. **Optimizers descend, so a likelihood is negated before stepping** — each of these headers exposes a `<model>_negative_log_likelihood` matching `LbfgsObjective` for exactly that. And **an infeasible parameter value returns a sentinel rather than aborting**: an optimizer probes points the model cannot evaluate, so a singular scale factor gives `INFINITY` and a zeroed gradient while programmer error still asserts. The check is on the computed likelihood as well as on the parameters, since a scale can be an ordinary positive number while the density built from it overflows — and a not-a-number reaching a line search is worse than an infinity, because every comparison against it is false in both directions. Both checks go through `MISNAN`/`MISINF` rather than comparing against `INFINITY` or a large number: under this project's `-ffast-math` the compiler settles such a comparison itself, and a test written that way accepted a not-a-number for as long as it existed. **When a simulator and an estimator both exist they read the same fields**, so their conventions cannot silently diverge; `sdloc_simulate` against `_sdloc_filter` is checked to `4e-16` over 200 periods in `tests/correctness/score_driven_location_correctness.c`.

A further rule binds the cache these models write. `<model>_fit_cached` reloads a fit the solver finished with, and **resumes one that stopped at its iteration cap** rather than returning it: a capped fit is a starting point, not an answer, and handing it back would mean a script could be rerun for ever without the estimate moving. Only the cap is resumed from, since a run that stalled or went non-finite did not run short of iterations and repeating it costs a whole fit for nothing — which is why the reason the search stopped is stored rather than derived from the convergence flag on load. What a chain of runs cost is then `total_niter`, the iterations summed over every run, beside `nruns`, how many runs there were; `niter` stays this run alone. **`LbfgsStatus` separates five outcomes and a boolean collapses them to two**, so the reason is kept per run in `run_status`, oldest first, with `status` the last of them — a run that hit its cap, one whose line search could not move and one whose objective stopped being finite call for three different responses, and only the first is worth resuming.

**A cache from an older format stays readable.** A file written before any of these fields existed loads, reports everything it does record exactly, and reports what it does not as not recorded rather than guessed: a fit that ran out of iterations and one that stalled both show `is_converged` 0, so no reason can be recovered from that flag. `<model>_fit_cached` hands such a fit back untouched rather than resuming it on a guess, since what one holds is a fit somebody has already paid for. A `run_status` whose length disagrees with `nruns`, or one carrying a value outside the five outcomes, reads the same way. A cache is a file a user can edit, so a missing field is a refusal to load rather than an assert, and a refusal leaves the caller's model untouched.
...
For every operation OpenBLAS provides at a size worth calling it at, the hot inner loop is OpenBLAS's, a hand-tuned, per-architecture assembly kernel this project does not attempt to match; `mat_mul` and every factorization/solve in `linalg/decomp.h`/`linalg/solver.h` are wrappers around it, not competing implementations. The qualification is the crossover below which the call costs more than the arithmetic inside it: a 5x5 by 5x1 product is fifty floating point operations and took 153 ns through `cblas_dgemm`, and 1375 ns when four threads issued it concurrently, because OpenBLAS keeps one buffer table per process. `linalg/mat.h`'s `mat_gemm` and `linalg/factor.h`'s `_trtrs`/`_potrs` dispatch below that crossover to a plain loop, and the crossover itself is measured in `tests/performance/small_blas_threshold.c` rather than chosen - see `MAT_GEMM_SMALL` and `TRSM_SMALL_N`. That is one dispatch decision in two kernels, not a licence to hand-write a second implementation of anything OpenBLAS covers. Those four constants are the only numbers in this library chosen from a measurement on the machine it was developed on, and they are the one thing here that a different CPU or a different BLAS can invalidate: `make bench-small_blas_threshold` re-derives all four on new hardware, and `docs/MATRIX_DOCUMENTATION.md`'s "The four dispatch thresholds are measured on one machine" says which parts of the reasoning still hold when they change and which do not. For the operations OpenBLAS does not cover, such as element-wise operations, reductions, and concatenation, keep the hot loop small, `restrict`-qualified, and stride-aware, and let the compiler auto-vectorize. Do not scatter performance-critical patterns throughout the codebase — optimize the few kernels that matter, and let the rest of the code be readable. Measure with `tests/performance/` before and after any change to a hot path.
...
**Do not duplicate a lower layer instead of calling into it.** Before writing a new loop or algorithm, check whether the layer below already provides it. `linalg/mat.h` and `linalg/decomp.h` delegate their heaviest operations to OpenBLAS rather than hand-writing a competing kernel, since OpenBLAS's hand-tuned, per-architecture assembly will beat portable C by a wide margin on anything it covers at a size where the call is worth making - see design principle 3 for the measured crossover below which it is not, and where the two kernels that dispatch around it live. The same discipline holds further up the stack even with no OpenBLAS routine in the picture: `frame/sql.h`'s evaluator reuses `mat.h`'s arithmetic and reductions instead of a second implementation, and `nn/mlp.h`'s training loop reuses `ad.h` and `solver/optimizer.h` instead of a bespoke one. If you find yourself reimplementing something a lower layer already does, the new code belongs in that lower layer, or should simply call it.
