...
`WHERE` itself does not go through `sql_eval`'s float-mask evaluation at all for the common case — it has its own bitmask-native evaluator (`sql_eval_mask`), ported from two specific techniques in Polars' real source (`crates/polars-compute/src/comparisons/simd.rs` and `crates/polars-arrow/src/bitmap/utils/slice_iterator.rs`, tag `py-1.38.1` — read directly, not approximated) rather than a general "make it columnar" rewrite: a `SqlBitmask` (one bit per row, computed via real AVX2 SIMD compare + a single `_mm256_movemask_ps` per 8-row chunk when available, a NaN-safe scalar loop otherwise) instead of `sql_eval`'s one-`mreal`-per-row mask, and a run-based row extraction (`sql_where_select_rows_fused`/`_mt`) that finds maximal runs of selected rows and bulk-`memcpy`s each run's whole row-major block in one call, instead of `sql_select_rows`'s column-outer, per-row index-array gather. Both are gated by measured thresholds (`SQL_WHERE_BITMASK_MIN_NCOLS`, `SQL_WHERE_PARALLEL_MIN_N`) below which the plain `sql_eval_num`-based approach this evaluator replaced is still faster — see docs/PERFORMANCE_BACKLOG.md item 5 for the full mechanism, the two real bugs found while building this (an AVX2 fast path that could never activate for any multi-column table, and a NaN-comparison bug under this project's own `-ffast-math` build — both are why `test_sql.c` now includes `test_where_nan_comparison` and `test_where_simd_kernel_engages_on_multicolumn`), and the measured numbers. `SELECT`-list expressions (not `WHERE`'s own top-level boolean tree) still go through `sql_eval`'s original per-row float-mask comparison logic, via the ordinary evaluation path described below.
...
A NaN in an `ORDER BY` key is defined to sort last (ascending) — matching pandas' own default `na_position='last'`. Both `sql_cmp_pair` (single-key) and `sql_compare_rows` (multi-key) route numeric comparisons through `sql_safe_order_cmp`, a three-way NaN-safe compare, rather than a plain `(a > b) - (a < b)`, which is `0` ("tied") whenever either side is NaN — a real transitivity violation (NaN "ties" with 5, NaN "ties" with -3, but 5 and -3 don't tie with each other) that could reorder two *real* values relative to each other, not just place the NaN itself somewhere unexpected, depending on input row order. Found via `docs/PERFORMANCE_BACKLOG.md` item 2's hash-based `GROUP BY` prototype (a different pre-sort row order than production's fed into the same, already-buggy comparator was enough to expose it) and captured in `test_order_by_nan_key_does_not_corrupt_real_order` before the fix landed. The separate radix-sort path (used above `SQL_RADIX_MIN_N` for a single numeric key) sorts by raw IEEE754 bit pattern instead of this comparator; a positive-signed NaN (what C's `NAN` produces) lands last there too, but a negative-signed NaN would land first — noted, not fixed, since real NaNs from actual computation are practically always positive-signed and changing that separately-verified bit-transform was out of scope here.

## Threading, and why OpenMP is optional here

The `WHERE` comparison kernel and both `GROUP BY` paths (group construction, and aggregation into per-thread accumulators) are parallelized with OpenMP above a row-count threshold. OpenMP is **not** a dependency of this project: the only ones allowed are OpenBLAS and what ships with GCC (see README.md's Pitfalls). It is used when the compiler was invoked with `-fopenmp` and stubbed out when it was not.

Whether that flag is present is not something this project controls, and assuming it is has broken the build once. `pkg-config --cflags openblas` returns `-fopenmp` when OpenBLAS was itself built against OpenMP and returns nothing when it was built against pthreads; Debian ships both under the same package name. So every OpenMP entry point the file uses is stubbed behind `#ifdef _OPENMP` to the values OpenMP itself defines outside a parallel region — `omp_get_max_threads()` to 1, `omp_get_thread_num()` to 0, `omp_get_num_threads()` to 1. The parallel sections are written as count-then-scatter keyed on the thread index, so one thread reporting index zero collapses them to a single serial pass over the whole range, computing the same answer more slowly.

**Stub every entry point the file calls, not only the ones it called when the fallback was written.** The fallback originally covered `omp_get_max_threads()` alone, which was all the `WHERE` path needed. The multi-threaded `GROUP BY` added `omp_get_thread_num()` and `omp_get_num_threads()` later and the fallback was not extended, so on a pthreads-OpenBLAS machine the build stopped at `undefined reference to omp_get_thread_num`. A missing stub does not quietly fall back to the serial path: the compiler discards an unsupported `#pragma` but leaves the ordinary function calls inside the block it guarded fully intact, so they reach the linker with nothing to resolve them.
