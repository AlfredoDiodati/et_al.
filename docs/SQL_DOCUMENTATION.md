...
Combined, all four fixes together: **faster than pandas at n=10,000, by roughly 2x** (down from an original 306x *slower*), and 1.5x slower at n=100,000/1,000,000 (down from 2.3x-2.4x after the quicksort fix alone, and from an original 1577x at n=30,000 that never even reached these sizes in any reasonable time). The remaining ~1.5x gap at n>=100,000 is tracked in `docs/PERFORMANCE_BACKLOG.md` (see docs/FRAME_DOCUMENTATION.md's Benchmark results for the full before/after, all four fixes measured separately).
