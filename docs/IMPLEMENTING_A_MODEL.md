...
Closed-form estimators. A model whose estimator has a closed form, such as least squares, runs no optimizer, so the rules above about optimizers do not apply to it and its fit has no convergence report: there are no iterations, no stopping status and nothing to converge. For the same reason it has no `<Model>Params` type and no `link` and `unlink`: they exist for an optimizer to step unconstrained values, and nothing is stepped. A constraint the model does have, such as a positive definite covariance, is checked where the value enters and reported in the fit notes. What it reports instead are fit notes, the facts about the computation a caller needs to judge the result. An estimator built on `ols` reports its status, which says whether the design had full column rank or the pseudo-inverse was used and at which column, the rank it used, and for each equation whether the residuals are exactly zero (`ols_residuals_are_zero`); any other step with a failure mode, such as a Cholesky factor of a residual covariance, reports its own status the same way. There is no refitting either. The solution cannot change from one run to the next, so there is nothing to resume: a stored fit is loaded and returned as it is, under the same fingerprint check as above, and `force_refit` is the only way to compute it again. The chain fields (`niter`, `total_niter`, `nruns`, `run_status`) do not exist for such a model.

Non-negotiable details:

- Optimizers descend, so a likelihood is negated before stepping.
- Report whether the fit converged and the final gradient norm, or, for a closed-form estimator, its fit notes. A fit whose status cannot be determined is not a result.
...
Other helper functions. A model may also have public functions that are not part of the fit and forecast pipeline, such as diagnostics, conversions between representations, or quantities derived from a fitted model for a caller's own use. They follow the same naming, ownership and documentation rules as the rest of the file.
