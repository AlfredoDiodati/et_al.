...
The same file also now covers `ad_solve`/`ad_chol_solve`/`ad_inv`/`ad_det` and `ad_huber_error`/`ad_logcosh_error`/`ad_swish`/`ad_lgamma`, previously exercised only by `test_ad.c`'s finite-difference checks, never timed. The multivariate log-likelihood gradient (`n=200` observations, `d=8`/`32`, one `ad_solve`/`ad_chol_solve`/`ad_matmul`-via-`ad_inv` call per observation - `ad.h` has no batch axis, see `mvgauss_dlogpdf_cov`'s own comment on why - versus JAX solving/factoring once for the whole batch, its natural idiom, so this is not an apples-to-apples vectorization comparison): `ad_solve` and `ad_chol_solve` are consistently ahead of jitted JAX (0.03-0.15x its time at both `d=8` and `d=32`) since the per-observation loop costs little more than the BLAS calls it makes. `ad_inv` is the interesting case - 7x *slower* than jit at `d=8` (JAX fuses the entire 200-observation batch into one XLA kernel small enough that per-call dispatch dominates on our side) but 20x *faster* at `d=32` (the per-observation `ad_matmul` cost grows enough to amortize that same per-call overhead, and reusing one `ad_inv` across all 200 observations starts paying off). The elementwise ops (`n=10,000`/`1,000,000`, one tape per call) show the mirror pattern: ahead of jitted JAX at `n=10,000` (0.27-0.82x for `huber`/`logcosh`/`swish`; `lgamma` is the exception at 3.8x, its double-precision `lgamma`/digamma per element costing more than JAX's compiled `gammaln` kernel can amortize even at this size) but 12-18x behind at `n=1,000,000`, where JAX's single fused vectorized kernel beats this file's straightforward scalar C loop at scale.
