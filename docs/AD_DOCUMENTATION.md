...
No broadcasting. Every op here requires exact shape matches between operands, the same contract `mat_add`/`mat_mul` etc. already have in `mat.h`. `dist/gauss.h`'s broadcasting is a separate, unrelated mechanism layered on top of plain `mat.h` calls - it has nothing to do with this file, and this file's ops don't inherit it.
