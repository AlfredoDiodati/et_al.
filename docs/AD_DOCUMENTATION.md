...
Node *ad_triangular_solve(Tape *t, Node *L, Node *b, char trans)  /* L lower; trans 'N' or 'T' */
...
`ad_matmul_backward` computes `Abar += Cbar*B^T, Bbar += A^T*Cbar` (table 3 "matrix product") through `mat_gemm`, `TransA`/`TransB` telling the BLAS kernel to read an operand transposed rather than materializing `B^T`/`A^T` as a separate matrix first, and `beta=1` accumulating straight into `a->grad`/`b->grad` rather than into a scratch buffer `ad_accum` then adds in. Earlier this called `mat_T` and `mat_mul` (an allocation and a full copy each) for both terms. The forward value goes through the same `mat_gemm`, into a pooled buffer, so the node costs no allocation of its own; unaffected by shape, including the non-square case (`test_matmul_nonsquare_fd`), which a square case cannot fully exercise since `Abar`'s and `Bbar`'s `gemm` calls use different transpose flags and a square shape cannot distinguish a transposed-dimension mistake in one from a compensating mistake in the other. Benchmarked before/after in Benchmark results below - about 1.1x-1.6x depending on `n`.

All three products - the forward one and the two adjoints - go through
`mat_gemm` rather than `cblas_?gemm` so that the small-size dispatch in
`linalg/mat.h` applies to them. That matters more here than anywhere else in
the library: a score-driven filter issues these three per period, at the
dimension of the observation vector, which is where a `gemm` call costs more
in dispatch than in arithmetic and where four concurrent threads serialize
...
Neither pass allocates. The forward substitution and the backward's two solves work in a stack buffer while `n <= AD_STACK_N` (64), the same trick and the same reason as `linalg/factor.h`'s `POTRI_STACK_N`: a filter calls this once per period at the dimension of one observation, where a `malloc` and `free` pair costs about as much as the arithmetic between them. The backward pass no longer calls `vec_chol_solve`, which allocated its own result, and solves `L y = b` then `L^T x = y` itself — `y` is the intermediate of the first half and is exactly the `y = L^T x` the rank-1 update below needs, so writing the two halves out separately removes the recomputation as well as the allocation.

`Lbar`'s formula is computed as a rank-1 update rather than the `n`x`n` outer product and `O(n^3)` matmul against `L` that this line's arithmetic literally reads as: `-2*qbar*x*x^T*L = -2*qbar*x*(x^T L)`, and `x^T L` is `y^T` for `y_j = sum_{i>=j} L[i][j]*x[i]` (the sum restricted to `L`'s nonzero lower-triangular entries, since the rest contribute zero regardless), computed once as an `n`-length vector, so `Lbar[i][j] += -2*qbar*x[i]*y[j]` for `j <= i`. `O(n^2)` throughout instead of `O(n^3)`, and in a stack buffer rather than an `n x n` allocation. Same arithmetic, verified by the same finite-difference and pair-comparison checks in `test_chol_quadform` this function already had - not a re-derivation, and this file's own comment on this function's earlier wrong derivation (see below) is exactly why those checks were run again rather than trusted by inspection alone.

Measured at 4.43x, 4.13x and 2.83x for `K` = 3, 6 and 12 against the pair, on a tape holding 600 quadratic forms, best of 7 interleaved rounds, float32, Intel i5-7400, `openblas_set_num_threads(1)` — see `tests/performance/bench_chol_quadform.c`, which also reports the two arms' worst gradient difference before timing either. The figures this replaces (1.30x, 1.29x, 1.27x on an AMD Ryzen 7 4800H) predate two changes: the stack scratch and split solve above, and `_trtrs`/`_potrs` dispatching small triangles to substitution instead of `?trsm`. The pair (`ad_dot` plus `ad_chol_solve`) gains from the second of those too but still allocates its own result and materializes the `n x n` outer product, which is where the widened gap comes from. Earlier still, the benchmark carried its own LAPACKE-based prototype of the function (1.23x, 1.18x, 1.13x); it now times the shipped one. Not from the TOMS paper, which has no quadratic-form row; derived from the pair it replaces.
...
`ad_triangular_solve(t, L, b, trans)` differentiates `vec_triangular_solve(L, b, 'L', trans, 'N')`: `L*x = b` for `trans == 'N'`, `L^T*x = b` for `trans == 'T'`. `uplo` is fixed at `'L'` rather than being a parameter, because every caller so far parametrizes a scale matrix by its lower Cholesky factor and wants the *half*-solve `Sigma^-1/2 v` rather than the full `Sigma^-1 v` that `ad_chol_solve` gives — that half-solve is what a score-driven filter scales its score by, and neither `ad_chol_solve` (a full system) nor `ad_chol_quadform` (a scalar, not the vector) produces it. `sd/score_driven_location.h` is the first caller.

Backward is Table 7's `getrs` row restricted to a triangular parametrization: with `z = vec_triangular_solve(L, xbar, 'L', opposite trans, 'N')`, `bbar += z` and `Lbar += tril(-z*x^T)` for `trans == 'N'`, `tril(-x*z^T)` for `trans == 'T'`. Only the lower triangle is accumulated, the same restriction `ad_chol_solve` applies for the same reason: the upper triangle is structurally zero, not a parameter. `trans` rides on the node's `aux` field.

Checked in `tests/correctness/test_ad.c` against central finite differences of an independently written reference loss, in both transposes and on a `3x3` rather than a `2x2` (where an off-by-one transpose in the outer product is far easier to pass by accident); on the invariant that the strict upper triangle of `L`'s gradient stays exactly zero; on the forward residual `L*x - b` and `L^T*x - b`; and — the check that is not a restatement of the formula — against `ad_chol_solve` itself, since `ad_triangular_solve(L, ad_triangular_solve(L, b, 'N'), 'T')` solves `(L*L^T) x = b`, and the two ops derive their adjoints separately.
