...
Implemented: general dense arithmetic (`add`, `sub`, `scale` by a constant, elementwise `mul`/`div`, `exp`, `log`, `pow` by a constant exponent, `tanh`, `identity`), `sum` and `dot` reductions, `matmul`, `squared_error` (a `Criterion`), and - the part that needed `linalg/decomp.h`/`linalg/solver.h` - `solve`, a Cholesky-factor solve, `det`, and matrix `inv`.
