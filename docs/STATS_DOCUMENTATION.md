...
Its first concrete consumer is the test suite's independence checks on `random/random.h` and the `dist/` samplers (see Testing below), but the functions are ordinary econometrics primitives — the empirical moments any method-of-moments estimator, residual diagnostic, or Ljung-Box-style serial-correlation check starts from.
...
**Double accumulation, mreal results.** All internal accumulation is in `double` regardless of the `mreal` build — the same policy as `special.h` and `random/random.h`, and for the same reason: summing 1e5+ `float` values in `float` loses digits the statistics actually need, and double accumulation makes a statistic of the same data agree across both precision builds up to the storage rounding of the inputs. `mat.h`'s `mat_mean` remains the core's `mreal` arithmetic reduction; `stats_mean` is the statistical counterpart built on this file's accumulation policy, not a competing implementation (the "do not duplicate a lower layer" pitfall is about hot kernels with one right answer; here the two contracts genuinely differ, and the test suite checks they agree to `mreal` tolerance).
...
- No weighted variants, no NaN-skipping variants — callers with missing data clean it first (`frame/` is the layer for that), and `mat_all_finite` (`linalg/mat.h`) is the check. What a `NaN` does on the way through this file is fixed by rule rather than by function: the accumulating functions let it reach the answer, and everything that sorts or produces a verdict asserts instead - the verdict half covering `inference/unit_root.h`, `inference/cointegration.h` and `inference/mcs.h` as well as this file. The reasoning and the full table are in `docs/FRAME_DOCUMENTATION.md`'s note on missing values; `tests/integration/join_missing_values.c` holds it in place. The short version is that a sort cannot carry a `NaN` — every comparison against one is false, so the partition is not an ordering — and before the guard `stats_median` returned 59.5 on a sample whose complete-case median was 49.5, with nothing in the return value to say so.

  What the guards cost, the same source built with and without `NDEBUG`, 41 rounds of a Gaussian sample, float64, `-O3 -march=native -ffast-math`, `OPENBLAS_NUM_THREADS=1` (best and median-of-rounds agreed within 3%):

  | n | `stats_median` guarded | unguarded | overhead |
  |---|---|---|---|
