...
- Malformed input (bad escape, mismatched brackets, trailing garbage, a truncated file) makes `json_parse` and `json_parse_file` return `NULL`, having freed whatever they had built. The text comes from files a user can truncate or edit, and the model caches in `sd/` and `varima/` refuse such a file rather than abort, which they could not do while the parser asserted: a truncated cache used to stop the program inside the parser. A file that cannot be opened is still an assert; the caches probe for it with `fopen` first. A caller for which a bad document is programmer error asserts on the `NULL` itself, as `nn/mlp.h`'s `mlp_load` does. `tests/correctness/test_json.c` checks every malformed case at the top level and nested inside a larger document, where the levels above the failure must free what they had built.
