# solver/optimizer.h - generic pluggable optimizer interface
...
Three pieces, matching `solver/adam.h`'s `adam_optimizer_init`/`adam_optimizer_step`/`adam_optimizer_free` pattern (or, for a minimal from-scratch example independent of Adam entirely, see `tests/correctness/test_optimizer.c`'s hand-rolled vanilla SGD):
