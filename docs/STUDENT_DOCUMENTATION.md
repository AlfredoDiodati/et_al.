...
The `nu`-dependent normalization constant (`student_lognorm`) is `special_lgamma_diff(nu/2, 1/2) - (log(nu) + log(pi))/2`, computed in **double regardless of `mreal`** — a deliberate, documented exception to the `MEXP`/`MLOG` macro discipline. Both `lgamma` terms grow like `(nu/2)*log(nu)` while their difference stays `O(log nu)`, so at `nu = 1e6` each term is ~3e6 and a float subtraction loses every significant digit of the O(10) difference.

Double alone does not save it. Evaluating that difference as `lgamma((nu+1)/2) - lgamma(nu/2)` loses nine digits by `nu = 3e6` and returns an integer by `nu = 1e15`, in double, which is why it now goes through `special_lgamma_diff` — see that function's entry in `docs/SPECIAL_DOCUMENTATION.md` for the restructuring and the measurement. In the general path a `1x1` `nu`'s constant is computed once before the loop; a per-element `nu` pays one `special_lgamma_diff` per element (see Known limitations).
...
The score with respect to the degrees of freedom — the piece that makes `nu` fittable by gradient alongside `loc`/`scale`. `psi` is the digamma function from `special.h` (`special_digamma`), which exists precisely because the C standard library provides `lgamma` but not its derivative. The `nu`-only part (`student_dlognorm_dnu`) is `special_digamma_diff(nu/2, 1/2)/2 - 1/(2 nu)`, computed in double throughout, same cancellation reasoning as `student_lognorm` one derivative up: the digamma difference is `~1/nu` against terms of size `~log(nu)/2` each, and taking it as a subtraction of two `special_digamma` calls returns `2.13e-14` where the answer is `2.50e-14` at `nu = 2e14` — besides which `special_digamma`'s own series is truncated at ~1e-11, larger than the whole difference past about `nu = 1e10`.
