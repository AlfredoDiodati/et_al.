...
mreal mat_norm(Mat m, char kind) // norm of m; kind: 'F'/'E' Frobenius (cblas_?dot(x,x) then sqrt), '1' one-norm, 'I' infinity-norm, 'M' max abs element (LAPACK ?lange)
```
