...
`random/random.h` is the general randomization engine every `dist/` sampler draws from: an explicit-state generator plus the variate primitives (uniform, standard normal, gamma, and a uniform random permutation) that distribution-shaped sampling is built out of. It is the base file of `random/`, the directory holding the bit generator and the sampling schemes built on it, and it depends on nothing at all — not even `linalg/mat.h` — and the `Mat`-valued, parameterized samplers (`gauss_sample`, `student_sample`, `mvgauss_sample`, `mvstudent_sample`) live in the `dist/` files, which call down into this one: the same include direction as every other layer, and the same split as "the engine knows nothing about `loc`/`scale`/`cov`". The `Rng` type with `rng_*` function prefixes follows `ad.h`'s `Tape`/`tape_*` precedent of naming functions after the type they operate on.
...
That last guarantee is about the `double`, and it does not survive a cast. The largest value the function can return, `1 - 2^-53`, rounds to exactly `1.0f` when stored as a `float`, which happens once in about 33 million draws. Nothing in `dist/` depends on the stored value staying below 1, but a caller that does — anything mapping a uniform onto a half-open range or an index — has to handle it, and `random/lhs.h` is the one place in the tree that does (see `docs/LHS_DOCUMENTATION.md`).
...
`rng_permutation` writes a uniform random permutation of `0 .. n-1` into a caller-provided array of `n` ints, by Fisher-Yates: walk from the back, swapping each position with a uniformly chosen one at or before it. Given an unbiased `rng_below` that is the one shuffle reaching all `n!` orderings with equal probability, and it costs `n-1` draws and no allocation. It was added for `random/lhs.h`, whose Latin hypercube design is one independent permutation per dimension (see `docs/LHS_DOCUMENTATION.md`), and it is the general primitive behind any resampling scheme that needs an ordering rather than a bootstrap's draw-with-replacement.
