...
`mlp_init`: `sizes[0]` is the input dimension, `sizes[1..n_sizes-1]` are each layer's output dimension in order - so `n_sizes-2` hidden layers and `n_sizes-1` weight/bias layers total. E.g. `{10, 32, 16, 1}` is 10 inputs, two hidden layers (32 then 16 units), 1 output. Weights are Glorot-uniform-initialized by drawing from `rng` (`random/random.h`'s explicit-state PCG64 generator, `docs/RANDOM_DOCUMENTATION.md`) - the caller constructs it (`rng_new(seed, stream)`), so there is no hidden global RNG state anywhere in this file: seeding, reproducibility, and independent streams (one per concurrent thread/run, say) are entirely the caller's to control. Caller must `mlp_free()`.
...
Three things about `mlp_fit` are only checked from `tests/integration/` (see `docs/INTEGRATION_DATA_SEAMS_DOCUMENTATION.md` and `docs/INTEGRATION_STRUCTURE_SEAMS_DOCUMENTATION.md`), because each is about a hand-off rather than about this file: that a `train_X` which is a strided window of a longer sample trains to the same weights as a contiguous copy of it (`frame_to_model.c`); that the `Optimizer` interface works here for something other than Adam, including per-tensor state allocated at the shape `mlp_fit` passes (`optimizer_swap.c`); and that an `MLPFit` still forecasts after both the `DataFrame` and the design matrix it was built from are freed (`pipeline_ownership.c`). The last of those also records what `mlp_fit` does on data with a hole in it: the loss is `NaN` from the first epoch and every weight ends `NaN`, but `epochs_run` is the full count and nothing in `MLPFit` says the run was meaningless.
