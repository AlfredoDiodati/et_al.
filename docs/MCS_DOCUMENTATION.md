...
At a thousand models over a thousand observations with 2000 draws and blocks of 20, one pass on 16 cores, the shipped header takes **2.96 s and 103.0 MiB under `MCS_TR`** and **0.88 s and 23.1 MiB under `MCS_TMAX`**. Before change 1 the `MCS_TR` run would have allocated 12.0 GiB and taken days; before change 4 the `MCS_TMAX` run was about `1.0e12` serial gathers, on the order of twelve minutes by arithmetic. Neither of those two was run.
...
- **The two HAC variants run on one thread.** Under the bootstrap variance both statistics split their gather, their spread accumulation and their per-round scan across threads under `#ifdef _OPENMP`, with the resamples still drawn serially, and the full result was checked byte-identical at 1, 3 and 16 threads. The general path the HAC variants take has no pragmas. Parallelising it was rejected once for the size of the index buffer it needed; with draws held as block starts that objection no longer holds, and it has not been measured again. `docs/MCS_PERFORMANCE_DOCUMENTATION.md` has both.
