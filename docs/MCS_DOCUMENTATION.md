...
The check that file exists for is the last one. `mcs()` allocates its working memory once, sized for the first round's series count, and reuses it as the set shrinks and the count falls — under `MCS_TR` at ten models the first round forms 45 differential series and the last forms one. The test writes the elimination loop out again over the public primitives with one difference, allocating each round's scratch at that round's exact size, and requires the two to agree *bit for bit*: the same survivors, the same elimination order, the same per-model p-values, the same `final_pvalue`. Not to a tolerance — both run the same arithmetic in the same order off the same `Rng`, so anything but an exact match is a difference in behaviour rather than in rounding. Eight panels cover both statistics and all three variances, and `STRESS=1` repeats it at 20 models and 500 resamples. If the answer ever comes to depend on how the scratch was sized — a stride taken from the allocation rather than from the round, a buffer read past the current round's extent — the two disagree, and nothing else in the suite would see it. That is the failure a rewrite aimed at spending less memory introduces, which is why it is a correctness test here rather than left to `tests/performance/mcs_candidates.c`, which measures the memory.
...
There is still no `bench_mcs.c`/`bench_mcs.py` pair timing this header against `arch.bootstrap.MCS` — see the limitation below. What exists is `tests/performance/mcs_candidates.c`, which answers a different question: does a candidate rewrite of this header return the same confidence set as the shipped one, and does it cost less to get there.

```bash
make bench-mcs_candidates MCS_CANDIDATE=inference/mcs_lowmem.h
STRESS=1 make bench-mcs_candidates MCS_CANDIDATE=inference/mcs_lowmem.h   # adds the two widest cases
```
...
`inference/mcs.h` is one header of `static inline` functions behind a `#pragma once`, so a candidate and the shipped version cannot both be included in one translation unit. They can be compiled into two units and linked into one binary, since every symbol in either is internal to its own unit. `tests/performance/mcs_arm.c` is that unit, compiled twice with different `-D` flags; `tests/performance/mcs_candidates.c` drives both and never includes this header at all. What crosses between them is in `tests/performance/mcs_arm.h` and holds no type from here: a candidate is free to change the layout of `MCSResult` or `MCSScratch`, and a struct whose two definitions disagree passed between two units is undefined behaviour rather than a benchmark.

Agreement is checked before cost, and it decides the exit status: a disagreeing case makes the binary exit nonzero, a faster arm does not make it exit zero. The discrete answer — who survived, in what order, whether the procedure stopped on evidence, and the model names the result carries its own copies of — must match exactly, because a confidence set has no floating tolerance. The p-values, t-statistics and Diebold-Mariano numbers are compared to `1e-12` relative, which leaves room for a reassociated sum and none for a different answer: a p-value is a count of exceedances over `opt.bootstrap` draws, so the smallest real disagreement it can express is `1/bootstrap`, and any difference in that block is reported separately as the whole number of bootstrap draws that changed side. Every arm is also checked against its own earlier rounds, which is what catches a candidate that reads uninitialized memory and so disagrees with itself.

Memory is counted rather than sampled. `mcs_arm.c` redirects `malloc`, `calloc`, `realloc`, `aligned_alloc` and `free` at the preprocessor to counting wrappers before including the header under test, so every allocation in that translation unit passes through one counter and every release through its partner, and the counter is marked immediately before the `mcs()` call so the loss table built above it is excluded. The number that comes out is exact and repeats to the byte; peak resident set size does not, being rounded to pages, dependent on what the allocator returns to the kernel, and inclusive of the process image. It was checked against the arithmetic: at `T = 120`, `M = 34`, 2000 draws under `MCS_TR` the counter reports 9,597,336 bytes and the buffers `mcs()` allocates sum to 9,597,336.

Where that memory goes, at the default variance, measured on the float32 build:

| case | T | M | draws | statistic | peak | time |
|---|---|---|---|---|---|---|
| `tmax_hac_resample` | 250 | 5 | 500 | Tmax | 34.5 KiB | 3.9 ms |
| `tmax_bootstrap` | 250 | 5 | 2000 | Tmax | 112.7 KiB | 6.2 ms |
| `tr_bootstrap` | 300 | 8 | 1000 | TR | 328.8 KiB | 18.7 ms |
| `tmax_long` | 1500 | 12 | 1500 | Tmax | 592.6 KiB | 137 ms |
| `tr_wide` | 200 | 16 | 1500 | TR | 1651.2 KiB | 140 ms |
| `tr_wide_stress` | 120 | 34 | 2000 | TR | 9372.4 KiB | 1025 ms |

Two things are visible in that column, and they are different problems. The bootstrap draw buffer `MCSScratch.bmean` is `8 * opt.bootstrap * mcs_n_series(stat, M)` bytes and grows quadratically in `M` under `MCS_TR`: it is 85% of `tr_wide`'s peak and 93% of `tr_wide_stress`'s. But at long `T` and few models it is not the largest block at all — `mcs()` also holds two `T x M0` copies of the loss table, `all` and `active_losses`, and those are 281 of `tmax_long`'s 593 KiB against `bmean`'s 141. `MCS_VARIANCE_HAC_RESAMPLE` allocates no draw buffer whatever, which is the 34.5 KiB row: the same shape costs 112.7 KiB under the default variance.

Timing follows README's protocol for it. The two arms alternate within each round in the order A B B A, so both orderings of a case are measured and a machine warming up over the run cannot be read as one arm being faster; the first round is discarded as a warmup. A case reports the best time each arm reached and, separately, the mean within-pair ratio under each ordering. Naming a winner needs the effect to clear a floor of 5%, which is measured rather than chosen: three runs of the binary with both arms built from `inference/mcs.h`, seven cases each, put the largest ratio between two copies of the same code at 1.032, with the other twenty observations inside 1.5%.

That it detects a difference was checked with a deliberately wrong candidate rather than argued: a copy of `inference/mcs.h` with the bootstrap variance divided by `opt.bootstrap - 1` instead of `opt.bootstrap`, one character of change. The four cases using `MCS_VARIANCE_BOOTSTRAP` came back `DIFFERS` at deviations of `2.5e-4`, `5.0e-4`, `3.3e-4` and `3.3e-4` — which are `1/(2B)` at each case's draw count, the relative change a factor of `B/(B-1)` makes to a standard error — and the three cases under the two HAC variances, which never reach that line, stayed identical. Worth noting what that run did *not* show: not one p-value moved and not one confidence set changed, because the observed statistic and every bootstrap statistic are scaled by the same factor and the exceedance counts are unaffected. A harness comparing only which models survived would have called that candidate correct.

Running it with no `MCS_CANDIDATE` builds both arms from `inference/mcs.h`, which is the harness checking itself: two objects from the same source against the same header must agree bit for bit on every case and time within noise, and a run reporting anything else is measuring the harness. Results go to `out/mcs_candidates_report.txt`. It is not part of `make test` or `bench.sh`, like the other standalone design-space benchmarks in the Makefile.
...
- **The draw buffer is the whole memory footprint at large `M`.** `MCSScratch.bmean` holds every resampled mean of every series at once, `8 * opt.bootstrap * mcs_n_series(stat, M)` bytes, because the draws are needed twice: once to form the bootstrap variance and again to reduce each draw to a statistic. Under `MCS_TR` that is quadratic in `M` and reaches 8.6 MiB at `M = 34` with 2000 draws. `docs/PERFORMANCE_BACKLOG.md` item 13 has the diagnosis and what a two-pass or online alternative would cost.
