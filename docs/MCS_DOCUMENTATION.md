...
Neither test fits anything. They are statistics of forecasts somebody else already produced, which is why this is a core-tier header rather than a model-tier one exposing the README's fit/forecast API. It is a standalone root-level header (`<noun>.h`, per README's naming policy) and includes `frame/frame.h` (the input type), `linalg/mat.h`, `random/random.h` (the bootstrap's draws), `special.h` (the normal tail), and `stats.h` (the HAC variance the two HAC variants and `dm_test` use).
...
void mcs_fwrite_rounds(FILE *f, const DataFrame *losses, const MCSResult *res);
DataFrame mcs_pvalue_frame(const DataFrame *losses, const MCSResult *res);
DataFrame mcs_round_frame(const DataFrame *losses, const MCSResult *res);
...
They take the whole `MCSOptions` rather than a statistic and a lag because under `MCS_VARIANCE_BOOTSTRAP` a t-statistic is not a function of the data alone: its standard error comes from the resamples, so the block length, the draw count and the stream all enter it. They draw from `rng_new(opt.seed, opt.stream)` in the order `mcs` does, so with the same options they run `mcs`'s first round on the same resamples and name the same worst model — `mcs_worst(&L, o) == mcs(&L, o).elimination_order[0]` and `mcs_statistic(&L, o)` matches `round_statistic[0]`, both checked under all three variances. The statistic matches to within one unit in the last place rather than bit for bit: it is the same expression inlined into two different callers, `-ffast-math` lets the compiler round the copies differently, and under the two HAC variants the two have been measured one unit apart. Under the two HAC variants no resampling happens in them at all.

`mcs_round` is one equivalence test, on differentials the caller has already built into `MCSScratch.d` or — under the bootstrap variance, for a caller that has filled `MCSScratch.losses`, `m0` and `active` — on the loss matrix directly, under either statistic. It fills `dbar`, `var` and `t`, writes the round's statistic to `stat_out` and returns its bootstrap p-value. Where it draws its own resamples it advances `rng` by `opt.bootstrap` block draws rather than reseeding; on the `losses` path it draws once, on the first round it is asked for, and every later round reuses them. `mcs_scratch_new` sizes the working memory for the first round's series count; `keep_draws` is `opt.bootstrap` when the round divides both sides by one standard error per series, `0` under `MCS_VARIANCE_HAC_RESAMPLE`, which reduces each draw as it is made. Together they are what a caller needs to write a different elimination loop without allocating inside a bootstrap.

The header's lower layer is public too, by its own convention — the only internal functions carry a leading underscore, `_mcs_scratch_alloc` and `_mcs_models_from_series`:

```c
void   mcs_gather(const DataFrame *losses, double *out);
void   mcs_build_diffs(const double *losses, int n, int m, MCSStat stat, double *d);
int    mcs_worst_from_tstats(const double *t, int m, MCSStat stat, double *rowmax);
double mcs_center(const double *s, int n, double *out);
double mcs_floor_var(double v);
double mcs_hac_var_mean(const double *s, int n, int hac_lag, double *scratch, double *mean_out);
double mcs_tstat(const double *s, int n, int hac_lag, double *scratch, double *mean_out);
double mcs_reduce(const double *t, int k_count, MCSStat stat);
int    mcs_name_width(const DataFrame *losses);
```

`mcs_gather`, `mcs_build_diffs` and `mcs_worst_from_tstats` are the rest of what a caller's own elimination loop needs: `tests/correctness/mcs_primitives.c` writes `mcs`'s loop out with these three, `mcs_scratch_new`, `mcs_round`, `mcs_scratch_free`, `mcs_effective_hac_lag`, `mcs_n_series` and `mcs_n_models`, and nothing else from the header. The other five are the arithmetic those are built from and `mcs_name_width` is the report writers' column width — tested directly, but rarely a reason to call them.
...
**The rounds.** The procedure runs `n_rounds = m0 - 1` rounds, numbered from 1, and drops one model in each. `round_eliminated`, `round_statistic` and `round_pvalue` hold, at index `round - 1`, the model that round dropped, its observed test statistic and its own bootstrap p-value. The round p-values are not `pvalue`: an MCS p-value is their running maximum, so once one round comes out below an earlier one its own p-value is gone from `pvalue` and survives only here. `elimination_round[j]` is the same information read per model — the round that dropped model `j`, `0` for the one model no round drops — and it is set for the models in the set too. For a model outside the set it is the round that removed it; for one inside, the round that would have removed it had the procedure not already stopped, which is the round Definition 4 reads that model's p-value from, so `pvalue[j]` is the largest `round_pvalue` up to and including round `elimination_round[j]`. `decided_round` is the round whose test was accepted, `0` when none was; the models outside the set are exactly the ones dropped before it, so `elimination_order[i] == round_eliminated[i]`.

**What the run was.** `options` is a copy of the `MCSOptions` the run was made with and `n_obs` the sample length, so a result says which `alpha` its set is a confidence set at, how many resamples a p-value is a fraction of, and how much data stood behind it — the last of which decides how far the set should be believed, per `docs/MCS_RELIABILITY_DOCUMENTATION.md`, and none of which the set carries on its own. It also makes Theorem 4 checkable on a result alone: `pvalue[j] >= options.alpha` exactly when `mcs_in_set(res, j)`.

`MCSResult` owns its nine arrays; free with `mcs_free`, which is safe to call twice.

### Reporting

A finished result is a struct of arrays, and every application turns it into the same tables. `mcs_fwrite_report` writes the per-model one to an open stream — each model's average loss, MCS p-value and the round that dropped it, which of them survived, the order the rest left in, and whether and in which round the set was decided on evidence. `mcs_fwrite_rounds` writes the per-round one — each round's size, statistic, own p-value, the running maximum that becomes its dropped model's MCS p-value, and the model, with the deciding round marked. From `examples/mcs_example.c`, squared-error loss:

```
squared error loss
  model        mean loss      MCS p  round  in set
  roll22        54.90755      0.244      4  yes
  roll66        57.76630      0.041      2  
  ewma94        53.46574      1.000      -  yes
  ewma97        55.00640      0.244      3  yes
  expanding     62.96091      0.009      1  
  eliminated, worst first: expanding roll66
  set decided by an accepted test: yes, in round 3 of 4 (p = 0.244 against alpha = 0.050)
  from round 3 on every model dropped is still in the set; those rounds run only to give it its MCS p-value
  round models   statistic   p-value     MCS p  dropped
      1      5      3.5580     0.009     0.009  expanding
      2      4      2.6555     0.041     0.041  roll66
      3      3      1.4595     0.244     0.244  ewma97     decided the set
      4      2      1.4455     0.120     0.244  roll22
```

The last line is why the round table exists: round 4's own p-value is 0.120, below round 3's, so it is invisible in the MCS p-value column, where `roll22` carries round 3's 0.244.
...
`mcs_fwrite_options` writes the configuration a run was made with — pass it `res.options` to report the run a result came from. `mcs_pvalue_frame` returns the per-model table as a `DataFrame` — model name, average loss, MCS p-value, `in_set` flag, `elimination_round` — and `mcs_round_frame` the per-round one — `round`, `models`, `statistic`, `pvalue`, `mcs_pvalue`, `decided`, `dropped` — for a caller who wants to write a csv or query the result rather than read it. Between them, an application producing a full report writes no loop of its own.
...
**A negative estimate is rejected, not clamped.** The rectangular window's spectral window is the Dirichlet kernel, which dips below zero, so `2*pi*f_d(0)` is not guaranteed non-negative. The paper: "in the rare event that a negative estimate arises, we treat it as 0 and automatically reject the null hypothesis of equal forecast accuracy." `dm_test` returns `status = DM_NEGATIVE_VARIANCE` with `pvalue = 0`, `stat = 0` and `std_error = 0`. In every case `mean_diff` is the sample mean loss differential `loss_a - loss_b`, and on `DM_OK` `std_error` is the standard error `stat` divides it by. `DM_ZERO_VARIANCE` (`stat = 0`, `pvalue = 1`) covers the separate case of two identical loss series, which the paper does not discuss because it cannot arise from two genuinely different forecasts.
...
**One set of resamples serves every elimination round.** The resampling is of observations, and an observation does not change when a model is eliminated, so nothing built from the draws depends on which models are still in the set. This is what the paper and the common implementations do, and redrawing per round would inject variation into the sequence of p-values that has nothing to do with the data. It is also what makes both statistics tractable at a large `M`: the per-model resampled deviations are computed once rather than once per round, and under `MCS_TR` so is every pair's spread. `MCS_TMAX`'s spreads depend on which models survive, so they are rebuilt each round, but from those deviations rather than from a fresh resample. Earlier versions of this header redrew each round, `MCS_TR` until Fix 2 and `MCS_TMAX` until Fix 4 of `docs/MCS_PERFORMANCE_DOCUMENTATION.md`, which reports the measured comparisons: no detectable shift in mean MCS p-value, and the same decision more often than two runs of one scheme under different streams give. Under the complete null a replication's coverage depends only on the first round, which sees the same draws under both schemes, so coverage there is unchanged by construction. `tests/correctness/mcs_size_and_power.c` gave the same coverage before and after each change, and after Fix 4 every coverage figure in `docs/MCS_RELIABILITY_DOCUMENTATION.md` was rerun and none moved.
...
**Scratch layout.** `mcs` allocates all its scratch once, through `_mcs_scratch_alloc` — the internal allocator `mcs_scratch_new` wraps, called directly because the shared path sizes its buffers by the model count rather than the series count and leaves out the buffers it never reads — sized for the first round's `M`, and reuses it as the set shrinks — there is no allocation anywhere inside the bootstrap loop. On the general path, which the two HAC variants take, that loop resamples every round and is nearly all the cost of the procedure; on the shared path the one-off tables, reading them out each round and the per-round scan share it. The differential array the general path builds is *series-major* (series `k` at `d + k*n`), unlike the row-major convention every `Mat` follows, because every mean, centering and HAC pass there walks one differential series contiguously and those passes are that path's whole cost. It is a plain `double` scratch buffer, not a `Mat`, so the row-major design principle does not apply to it. The two variances that divide both sides by one standard error per series also keep every resampled mean, because the draws are needed again after the variance has been formed from them; `MCS_VARIANCE_HAC_RESAMPLE` reduces each draw as it is made and keeps none. How many that is depends on the variance: `opt.bootstrap * mcs_n_series(opt.stat, M)` doubles under `MCS_VARIANCE_HAC`, but under the bootstrap variance `mcs` keeps one entry per *model* under either statistic — a pair's resampled mean is the difference of its two models', and a model's mean against the others is a combination of every model's — which under `MCS_TR` at a thousand models is a factor of five hundred, and `MCSScratch.d` is then not built at all. That path also forms its resampled means once for the whole run rather than once per round, which is possible because one set of resamples serves every round; see **Conventions and deliberate choices**. On both paths the draws are held a chunk at a time as block starts, one `int` per block, drawn serially and expanded into observations inside the gather, which is what lets a chunk's draws be split across threads.

**`MCS_TMAX` and rounding.** The "mean of the other models" is formed by subtracting each model's own loss from a total, `O(M)` rather than `O(M^2)`: per observation on the general path, and on the shared path per model mean and per resampled deviation. On losses that are identical across models this does not recover the original value bit-for-bit for every `M` — the differentials are rounding noise rather than exact zeros. It makes no difference on data with any actual variation, and the exact-zero case is covered by `MCS_TR`, where identical columns give identical sums and their difference is exactly zero on either path.
...
Seven files check this header, from hand-computed values through Theorem 1's coverage guarantee to a candidate-comparison harness. `docs/MCS_TESTING_DOCUMENTATION.md` indexes them with what each one answers and how long it takes, and describes every check in them.
...
The file is split into two halves and says so. The first manufactures the loss table out of a returns file — rolling windows, an EWMA recursion, a warmup period — and touches nothing in this header; in a real application somebody else's forecasting code sits there instead. The second is `main`, and it contains no loops and no hand-written tables, only calls into `inference/mcs.h` and the file handles they write through. That property is the example's real purpose: whenever an earlier draft needed a loop at the call site, the missing piece went into this header instead, which is where `mcs_fwrite_report`, `mcs_fwrite_rounds`, `mcs_in_set`, `mcs_fwrite_options`, `mcs_pvalue_frame`, `mcs_round_frame` and `mcs_effective_hac_lag` all came from. The example also writes `examples/out/mcs_example_qlike_rounds.csv`.

Its result is the point of the procedure: under QLIKE the set collapses to `ewma94` alone, while under squared error three of the five survive — `expanding` and `roll66` are eliminated in rounds 1 and 2 and round 3's test decides the set at `p = 0.244`. A confidence set is a statement about a loss function, not about the models on their own. The QLIKE collapse is also worth reading carefully, and the round columns are what make that possible: every test rejected down to the last model (`p = 0.007` in round 4), so `ewma94` is the set by exhaustion rather than by an accepted test.

## Benchmark results

`inference/mcs.h` still has no `bench_mcs.c`/`bench_mcs.py` pair timing it against `arch.bootstrap.MCS` — see the limitation below. What it has is `tests/performance/mcs_candidates.c`, a harness that compares a candidate rewrite of this header against the shipped one on agreement first and on time and allocated bytes second, and five changes that went through it:

1. Under `MCS_TR` with the bootstrap variance, a pair's resampled mean is the difference of two per-model resampled means, so one gather per model replaces one per pair. Bit-identical elsewhere, `5e-16` to `1.9e-15` relative on the changed cases, no p-value moved.
2. One set of resamples shared across every elimination round under `MCS_TR`, rather than a fresh set per round. This moves p-values by whole draws, so it was gated statistically.
3. Skipping the rows of a bootstrap draw that provably cannot exceed the observed statistic. Exact.
4. `MCS_TMAX` under the bootstrap variance moved onto the same per-model table and shared draws, and the draws held as block starts rather than index lists. Also gated statistically.
5. The two HAC variants split their draws across threads; under `MCS_TR` each pair's reciprocal standard error is formed once rather than once per round, the spreads are accumulated a cache-sized block of pairs at a time, and the row test of change 3 bounds a row by the models it actually pairs with and by each model's own largest reciprocal standard error, so one very noisy model no longer stops rows being skipped. Exact.

At a thousand models over a thousand observations with 2000 draws and blocks of 20, one pass on 16 cores, the shipped header takes **1.08 s and 42.1 MiB under `MCS_TR`** and **0.95 s and 23.1 MiB under `MCS_TMAX`**. Before change 1 the `MCS_TR` run would have allocated 12.0 GiB and taken days; before change 4 the `MCS_TMAX` run was about `1.0e12` serial gathers, on the order of twelve minutes by arithmetic. Neither of those two was run.

Against the header before change 4, both arms in one binary, best of six rounds, `float32` build, 16 cores: `MCS_TMAX` at 5 models over 250 observations with 2000 draws went from 6.29 ms to 1.28 ms, at 12 models over 1500 observations with 1500 draws from 138 ms to 1.29 ms, and at 60 models over 500 observations with 2000 draws from 1328 ms to 9.67 ms. `MCS_TR` under the bootstrap variance gained 1.01x to 1.55x, the HAC variants were unchanged at 1.00x, and every case used less memory. Over 200 paired replications the mean MCS p-value moved by `0.00005` (standard error `0.00046`) and `0.00074` (`0.00077`) on the two `MCS_TMAX` bootstrap cases, and coverage under `tests/correctness/mcs_size_and_power.c` was unchanged.

Change 5, against the header before it, the median of 6 alternating runs of the whole call on the same data: `MCS_TR` at a thousand models went from 3.57 s to 0.97 s, and with one model carrying twenty times the others' noise from 23.4 s to 1.08 s; in the harness `MCS_VARIANCE_HAC_RESAMPLE` at 5 models went from 4.08 ms to 0.93 ms and `MCS_TR` under `MCS_VARIANCE_HAC` at 8 models from 19.1 ms to 7.5 ms, for 80 KiB and 24 KiB more memory.
...
- **Coverage is below nominal at small samples with serially correlated losses.** Measured at 250 observations of AR(1) losses at `phi = 0.5`: 0.920 for `MCS_TMAX` and 0.890 for `MCS_TR` against a nominal 0.95, so the set is too small rather than too large. It is the moving block bootstrap's finite-sample approximation of the dependence, not the resample count — ten times as many resamples changes nothing — and it converges only as the sample and the block length grow together. `docs/MCS_RELIABILITY_DOCUMENTATION.md` has the sweeps and the practical reading.
- **Not compared against another library.** There is no `bench_mcs.c`/`bench_mcs.py` pair. The natural comparison is `arch.bootstrap.MCS` (which uses the stationary bootstrap, so the timing would compare implementations of different resampling schemes) or the JAX implementation this was translated from. On the general path — either statistic under a HAC variant — the cost is `opt.bootstrap * mcs_n_series * T` gather-and-sum operations per round; `MCS_VARIANCE_HAC` adds a HAC pass of `hac_lag * T` per series per round and `MCS_VARIANCE_HAC_RESAMPLE` one per series per draw. Under the bootstrap variance both statistics pay `opt.bootstrap * M0 * T` gathered additions once for the run; each round then costs `MCS_TR` a scan of up to `opt.bootstrap` times the surviving pairs, and `MCS_TMAX` of order `opt.bootstrap` times the surviving models for its spreads plus a scan of at most that. `tests/performance/mcs_candidates.c` above times this header against another version of itself, which is a different question and does not answer this one.
- **`MCS_TR` under the two HAC variants is still quadratic in `M` in both time and memory.** The factorisation above applies only to the bootstrap variance, because the two HAC variants divide each series by a standard error estimated from that series and so cannot reach a pair's number through its two models'. `MCSScratch.d` and `MCSScratch.bmean` are sized by the pair count there, and a `MCS_TR` run at a few hundred models under `MCS_VARIANCE_HAC` is not practical. The default variance is the one Hansen, Lunde and Nason use, so this is a limit on the variants rather than on the procedure.
...
- **Under `MCS_VARIANCE_HAC` every round draws fresh resamples.** The bootstrap variance draws once and every round reuses the draws; the HAC variant could too, since its null deviations are linear in each model's resampled mean, but that moves p-values by whole draws and has not been done. `docs/MCS_PERFORMANCE_DOCUMENTATION.md` has it under "Still open".
...
- **`mcs_pvalue_frame` and `mcs_round_frame` narrow back to `mreal`.** Their p-value and statistic columns are `DataFrame` storage, so a number written through either is subject to the build's element type again. That costs nothing for an MCS p-value, which is a bootstrap fraction with granularity `1/opt.bootstrap`; a round statistic loses digits past `float`'s seven under the default build, and the result keeps the full `double`; but a `DieboldMariano` p-value has no such export path and should be written from the struct directly.
