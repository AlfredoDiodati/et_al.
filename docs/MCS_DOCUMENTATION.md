...
- **Not compared against another library.** There is no `bench_mcs.c`/`bench_mcs.py` pair. The natural comparison is `arch.bootstrap.MCS` (which uses the stationary bootstrap, so the timing would compare implementations of different resampling schemes) or the JAX implementation this was translated from. On the general path — `MCS_TMAX`, or `MCS_TR` under a HAC variant — the cost is `opt.bootstrap * mcs_n_series * T` gather-and-sum operations per round; the two HAC variants add a factor of `hac_lag` to it, `MCS_VARIANCE_HAC` once per round and `MCS_VARIANCE_HAC_RESAMPLE` once per draw. `MCS_TR` under the bootstrap variance pays `opt.bootstrap * M0 * T` gathered additions once for the run and a scan of up to `opt.bootstrap` times the surviving pairs each round. `tests/performance/mcs_candidates.c` above times this header against another version of itself, which is a different question and does not answer this one.
...
- **Only `MCS_TR` under the bootstrap variance runs in parallel.** That path splits its gather, its spread accumulation and its per-round scan across threads under `#ifdef _OPENMP`, with the resamples still drawn serially so the answer does not depend on the core count. `MCS_TMAX` and both HAC variants run on one thread: parallelising the general path's gather needs each round's index blocks buffered first, and at 1500 observations of 12 models that measured 1.45x faster for a 384 KiB index buffer added to a 592 KiB working set, 65% more memory. `docs/MCS_PERFORMANCE_DOCUMENTATION.md` has both.
