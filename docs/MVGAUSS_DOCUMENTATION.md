...
`dist/mv/gauss.h` implements the multivariate Gaussian (normal) distribution: pdf, log-pdf, and the log-pdf's derivatives with respect to the mean vector and the covariance matrix. For an observation that is a whole matrix rather than a vector, and therefore carries one covariance per axis, see `dist/mv/matgauss.h` and `docs/MATGAUSS_DOCUMENTATION.md`. It is the first file in `dist/mv/`, the subfolder of `dist/` for multivariate distributions — distributions whose density couples the components of a vector-valued observation, as opposed to the element-wise scalar distributions of `dist/` itself (`dist/gauss.h`). The split is not cosmetic: the two kinds of file sit at different layers. An element-wise distribution needs only `linalg/mat.h`; a multivariate one needs a matrix factorization (here, the Cholesky of the covariance for the Mahalanobis quadratic form and the log-determinant), so `dist/mv/` files include `linalg/decomp.h`. This is exactly the future `dist/gauss.h`'s doc anticipated ("a multivariate Gaussian would need them"), now concrete.
...
Computed the standard numerically sound way: one Cholesky factorization `cov = L*L^T` (`mat_chol`) shared by all `n` observations, then `q_i = ||y_i||^2` where `L*y_i = x_i - loc` — a single triangular solve against all `n` right-hand sides at once, never an explicit inverse — and `log(det(cov)) = 2*sum(log(diag(L)))`, which cannot overflow the way `det` itself can. `mvgauss_pdf` is `exp` of the log-pdf (the log-pdf is the primitive, same direction as `dist/gauss.h`).

The solve goes through `linalg/factor.h`'s `_trtrs` rather than `cblas_?trsm` directly, so it takes the same small-size dispatch as every other triangular solve in the library: substitution when `d <= TRSM_SMALL_N` and `n <= TRSM_SMALL_NRHS`, `?trsm` otherwise. Measured on 2026-09-24, before and after that change, AMD Ryzen 7 4800H, 16 threads, OpenBLAS 0.3.33 OpenMP build, no thread variable set, one call timed in batches doubled until a batch takes 20 ms, best of 5 batches, 8 alternating pairs in each order, float64: `mvgauss_logpdf` went from 939 to 578 ns at `d = 2`, 100 observations; 7633 to 2120 ns at `d = 5`, 256; 9792 to 3123 ns at `d = 8`, 256; 37044 to 19121 ns at `d = 12`, 1000; 199333 to 171434 ns at `d = 12`, 4096. At `d = 13` and `d = 32`, where the dispatch still calls `?trsm`, the ratio stayed between 0.91 and 1.01. float32 gave 0.30x to 0.80x on the same five shapes. `mvstudent_logpdf` and `mvstudent_dlogpdf_nu` share the change. `tests/correctness/mv_density_dispatch.c` checks the densities against a long-double reference on both sides of the dispatch.
