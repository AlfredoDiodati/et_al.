...
The coordinator takes a whole range for itself, the same width a remote machine gets. An earlier version drew one task at a time locally, on the reasoning that its own work costs no round trip and batching it only delays the next look at the sockets; that was worth about 25% of throughput on the workload in Benchmark results below, where every task is a single-threaded arithmetic loop. It is the wrong trade for a task function that parallelizes its own loop, which is the shape this file is built around and the only way any machine here ever uses more than one core: a coordinator handed one task at a time has nothing to spread over its own cores and runs the whole local share on one. Taking a range does delay the next look at the sockets by however long that range takes, but a range computed on every core finishes far sooner than the same range would one task at a time on one.
...
What that file cannot cover is the workload anybody would actually distribute. Every task in it is a pure arithmetic loop, and a simulation is the case where being wrong leaves no trace: if the ranges handed to different machines all seed the same generator, every machine draws the same numbers and the quantile comes out of a sample a fraction of the size it claims. `tests/integration/distributed_simulation.c` runs a real one - a Gaussian random walk from `rng_new(seed, chunk->lo + j)` and its ADF statistic - serially and across forked workers, and requires the two to agree replication by replication and on the simulated 5 per cent critical value. It also runs a task that deliberately ignores its global index and requires the result to be detectably different, without which the comparison would pass just as happily against a library where every draw was identical, and it checks that a range reclaimed from a killed worker is recomputed to the same values rather than merely being accounted for. See `docs/INTEGRATION_STRUCTURE_SEAMS_DOCUMENTATION.md`.

The registry-order hazard that pairing suggests - two binaries registering the same number of tasks in a different order would pass the handshake's task-count check and then run each other's functions - does not exist in practice, and no test was written for it: `_cluster_hello_matches` also compares `_cluster_self_fingerprint`, an FNV hash of the whole executable, and two binaries with different registration order are different executables. The hole is only where `/proc/self/exe` cannot be read, where the fingerprint is 0 on both sides and the check degrades to protocol and ABI, which this file already documents.
...
- **The task registry is file-static**, the one piece of hidden global state in this library outside a caller's control. A worker has to reach a function chosen before it knew what job was coming, and there is nowhere else to put it. See `random/random.h`'s header for why hidden state is otherwise refused throughout this project.
- **No cross-machine benchmark.** Every number above is loopback on one PC. What two real machines on wifi actually achieve depends on their relative speed and the network, and is not measured here. `tests/performance/qvarma_cluster_fits.c` is the driver built for that measurement - a batch of t-QVARMA fits, OpenMP inside a range and this engine between machines - and it has been run on one machine and on two processes of one machine, but not yet on two. See `docs/QVARMA_DOCUMENTATION.md`. On one machine with no worker running it costs 5 to 6 percent against the same batch as a plain OpenMP loop, which is what a batch pays for being distributable at all.
- **Two machines built separately cannot pair, by design.** The handshake compares an FNV hash of `/proc/self/exe`, so the same source compiled on two machines gives two executables and they are refused. Deploying one binary through the daemon is the intended answer, and it makes the coordinator's build flags the ones both machines run: `-march=native` then targets whichever machine built it. `x86-64-v3` is AVX2, FMA and BMI2, which anything recent has, and it measured the same speed as native on `sd/qvarma.h`'s filter - see the `qvarma_cluster_fits_portable` target in the Makefile. A deployed binary also has to find its shared libraries and a new enough glibc on the other machine, neither of which the handshake checks.
