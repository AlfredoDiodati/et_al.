...
**Rolling mean** `tensor_rolling_mean(t, window, axis)`, the right-aligned mean over `window` consecutive elements along one axis, same shape as `t`, the first `window - 1` positions NaN; a view is handled as for `tensor_cumsum`. It runs on `linalg/mat.h`'s `_mat_rolling_mean_kernel`; see `docs/MATRIX_DOCUMENTATION.md`, "Rolling mean", for how each mean is summed, its accuracy against numpy and polars, and its timings.
...
**The batch loop takes threads only below the BLAS crossover.** Above it the work is inside OpenBLAS, which keeps one buffer table per process and slows sharply when several threads call it at once: this project measured 153 ns for one 5x5 by 5x1 product against 1375 ns for the same product issued from four threads (README design principle 3). Below the crossover the arithmetic is `mat_gemm`'s own loop, which shares nothing, so the batch is split. Which side of the crossover a product is on is asked of `linalg/mat.h`'s `_mat_gemm_runs_loop`, the same function `mat_gemm` dispatches on, so the batch is threaded exactly when every product in it runs as the loop. A separate copy of the rule here once lacked the tall matrix-vector clause (`MAT_GEMM_THIN`), and a batch of 64 products of `m x 8` by `8 x 1` at `m = 1000` ran 11x slower than it does now (float64, 16 threads, 12 alternating pairs in each order).
