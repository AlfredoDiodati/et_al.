...
/* Parallelizing anything below this many rows loses to thread-spawn
   overhead - OpenMP's own `if()` clause on the pragma makes this a
   runtime, not compile-time, decision (single-threaded fallback is
   automatic, not a separate code path), so one constant covers both the
   comparison kernel and the row-extraction scatter below. Not tuned
   precisely - a round number comfortably above the ~few-hundred-
   microsecond range where 16 threads' spawn/join cost would dominate
   the work itself, picked to be measured against, not assumed correct. */
#define SQL_V6_PARALLEL_MIN 200000

/* Measured against real pandas/Polars across a 4x3 grid of n in
   {1000,10000,100000,1000000} x ncols in {2,8,32}: v6's WHERE path
   (bitmask compute + fused run-extraction) is slower than production's
   plain sql_apply_where specifically at ncols=2, for every n below
   1,000,000 - the fixed cost of building/scanning a bitmask isn't
   amortized when there's only 2 columns' worth of data per row to
   bulk-copy. At ncols=8 and ncols=32, v6 won at every n tested,
   including n=1,000. ncols=3 through 7 were never measured, so
   SQL_V6_MIN_NCOLS is a conservative choice (only ncols=2 is
   confirmed-losing; grouping anything narrower than a typical panel
   width with it, not because 3-7 are known to lose too) rather than a
   precisely fitted cutoff - revisit if this is ever measured more
   finely. Only gates sql_apply_where_v6 - SELECT projection and ORDER
   BY are unmodified production code in v6 regardless of width, so there
   is nothing to fall back on there. */
...
/* ---------------------------------------------------------------------
   v6: parallelizes v5's fused run-detection+copy via a classic
   count-then-scatter pattern. Each of omp_get_max_threads() threads
   independently finds runs within its own contiguous, byte-aligned
   slice of the mask and records them into its own scratch array - fully
   independent work, no shared state, so this first pass is
   embarrassingly parallel. A cheap serial prefix-sum over each thread's
   selected-row count then gives every thread a non-overlapping output
   offset; a second parallel pass does the actual memcpy's, each thread
   writing only into its own precomputed region of out.numeric - never
   overlapping, so no synchronization is needed there either. Falls back
   to v5's single-threaded sql_select_rows_fused below
   SQL_V6_PARALLEL_MIN (thread spawn/join cost would dominate small
   work), same reasoning as the comparison kernel above. String columns
   and metadata stay single-threaded afterward (merging every thread's
   runs into one flat list first) - never the identified bottleneck, no
   need to parallelize them too. */
...
static DataFrame sql_apply_where_v6(const SqlExpr *where, const DataFrame *df) {
    ...
    /* Narrow table, not yet at the row count where the measured
       crossover happened - use production's own sql_apply_where
       instead (see SQL_V6_MIN_NCOLS's comment). */
...
static DataFrame sql_execute_v6(const SqlQuery *q, const DataFrame *df) {
    assert(!q->is_star && "sql_execute_v6 prototype: SELECT * not implemented");
    int has_agg = 0;
    for (int i = 0; i < q->n_items; i++)
        if (sql_expr_contains_agg(q->items[i].expr)) { has_agg = 1; break; }
    assert(!(q->n_group_by > 0 || has_agg) &&
           "sql_execute_v6 prototype: GROUP BY/aggregates not implemented (out of scope - see file header)");
...
static int check_query(const DataFrame *df, const char *query) {
    ...
    DataFrame got6 = df_sql_v6(df, query);
    int ok4 = df_numeric_equal(&ref, &got4);
    int ok5 = df_numeric_equal(&ref, &got5);
    int ok6 = df_numeric_equal(&ref, &got6);
    printf("  %-70s v4=%s v5=%s v6=%s (r=%d vs %d vs %d vs %d)\n", query,
           ok4 ? "MATCH" : "MISMATCH", ok5 ? "MATCH" : "MISMATCH", ok6 ? "MATCH" : "MISMATCH",
           ref.r, got4.r, got5.r, got6.r);
    df_free(&ref); df_free(&got4); df_free(&got5); df_free(&got6);
    return ok4 && ok5 && ok6;
}
...
int main(void) {
    ...
    /* n=2000 above is well below SQL_V6_PARALLEL_MIN (200,000) and
       always at ncols=8 - every check so far only exercises v6's
       single-threaded fallback (it just calls v5's sql_select_rows_fused
       directly) at one fixed width, never the actual parallel count-
       then-scatter path, and never a narrower/wider table. Both matter:
       a per-thread chunk-boundary or prefix-sum off-by-one could hide at
       one specific (n, ncols) combination and only show up at another -
       this project was explicitly told earlier in this session not to
       test a single fixed ncols again, so every check below sweeps both
       row count AND column count together. */
    printf("\ncorrectness (parallel path specifically, n >= SQL_V6_PARALLEL_MIN, swept over ncols too):\n");
    int nthreads_here = omp_get_max_threads();
    int par_sizes[] = { 200000, 200000 + 1, 200000 + nthreads_here - 1, 200000 + nthreads_here, 500000, 1000003 };
    int par_ncols[] = { 2, 4, 8, 16, 32 };
    for (size_t ci = 0; ci < sizeof(par_ncols)/sizeof(par_ncols[0]); ci++) {
        int ncols = par_ncols[ci];
        char select_all[256] = "SELECT c0";
        for (int j = 1; j < ncols; j++) { char piece[16]; snprintf(piece, sizeof piece, ", c%d", j); strcat(select_all, piece); }
        strcat(select_all, " FROM df WHERE c0 > -1000"); /* selects every row - exercises the all-0xFF-byte fast path across chunk boundaries, at this specific width */

        for (size_t pi = 0; pi < sizeof(par_sizes)/sizeof(par_sizes[0]); pi++) {
            DataFrame big = make_test_df(par_sizes[pi], ncols);
            printf("  n=%d ncols=%d:\n", par_sizes[pi], ncols);
            all_ok &= check_query(&big, "SELECT c0, c1 FROM df WHERE c0 > 0");
            all_ok &= check_query(&big, "SELECT c0 FROM df WHERE c0 > -3 AND c0 < 3");
            all_ok &= check_query(&big, select_all);
            df_free(&big);
        }
    }
    ...
    printf("\n=== benchmark: df_sql (production) vs df_sql_v4 (faithful port) vs df_sql_v5 (fused extraction) vs df_sql_v6 (+ OpenMP, %d threads) ===\n", omp_get_max_threads());
    BenchQ queries[] = {
    ...
    /* Both axes swept together, per this session's standing instruction
       not to fix ncols at one value - row count from small to large,
       column count from narrow to wide. */
    int sizes[] = { 1000, 10000, 100000, 1000000 };
    int ncols_sweep[] = { 2, 8, 32 };
    for (size_t ki = 0; ki < sizeof(ncols_sweep)/sizeof(ncols_sweep[0]); ki++) {
        int ncols = ncols_sweep[ki];
        for (size_t si = 0; si < sizeof(sizes)/sizeof(sizes[0]); si++) {
            int n = sizes[si];
            int reps = (n <= 1000) ? 200 : (n <= 10000) ? 60 : (n <= 100000) ? 20 : 5;
            DataFrame df = make_test_df(n, ncols);
            for (size_t qi = 0; qi < sizeof(queries)/sizeof(queries[0]); qi++) {
                double best_prod = 1e18, best_v4 = 1e18, best_v5 = 1e18, best_v6 = 1e18;
                ...
                for (int r = 0; r < reps; r++) {
                    double t0 = now_ms(); DataFrame o = df_sql_v6(&df, queries[qi].query); double t1 = now_ms();
                    df_free(&o);
                    if (t1 - t0 < best_v6) best_v6 = t1 - t0;
                }
                printf("  n=%-9d ncols=%-3d %-32s prod=%9.4fms  v4=%9.4fms(%.3f)  v5=%9.4fms(%.3f)  v6=%9.4fms(%.3f)\n",
                       n, ncols, queries[qi].label, best_prod, best_v4, best_prod / best_v4,
                       best_v5, best_prod / best_v5, best_v6, best_prod / best_v6);
            }
            df_free(&df);
        }
    }
