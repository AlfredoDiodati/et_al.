...
static int check_query(const DataFrame *df, const char *query) {
    ...
    DataFrame got = df_sql_v4(df, query);
...
int main(void) {
    ...
    printf("\n=== benchmark: df_sql (production) vs df_sql_v4 (faithful port) ===\n");
    BenchQ queries[] = {
    ...
    for (size_t si = 0; si < sizeof(sizes)/sizeof(sizes[0]); si++) {
        ...
        for (size_t qi = 0; qi < sizeof(queries)/sizeof(queries[0]); qi++) {
            double best_prod = 1e18, best_v4 = 1e18;
            ...
            printf("  n=%-9d %-32s production=%9.4fms  faithful=%9.4fms  ratio=%.3f\n",
                   n, queries[qi].label, best_prod, best_v4, best_prod / best_v4);
        }
        df_free(&df);
    }
