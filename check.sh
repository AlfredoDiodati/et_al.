...
SUITES="poly_correctness spline_design_correctness spline_basis_correctness spline_objects_correctness test_mat test_tensor test_tensor_serial cumsum_correctness rolling_mean_correctness hp_filter_correctness hp_filter_recovery test_mat_special test_decomp test_solver lstsq_rank_deficiency chol_singularity singularity_rule_comparison ols_pseudo_inverse_fallback hac_covariance_correctness ols_covariance_correctness ols_covariance_recovery lag_matrix_layout rank_rule_consistency test_special test_stats test_random test_lhs test_mcs test_mcs_variance mcs_primitives mcs_size_and_power test_broadcast test_gauss test_student test_mvgauss test_mvstudent test_matgauss test_matgauss_recovery mv_density_dispatch test_ad ad_tensor_gradients test_tape_reset test_adam test_optimizer test_cluster test_mlp test_frame test_csv csv_missing_values test_txt test_npy test_npz test_json test_sql test_join gzip_inflate gzip_deflate rdata_array_read adf_correctness kpss_correctness dfgls_correctness otto_correctness hlt_union_correctness hlt_break_correctness hhlt_correctness zivot_andrews_correctness johansen_correctness engle_granger_correctness maki_correctness qlr_test_correctness lbfgs_correctness score_driven_location_correctness qvarma_correctness qvarma_analytic_agreement qvarma_gaussian_limit qvarma_identification qvarma_fixed_parameter_fit var_correctness var_recovery lp_correctness lp_recovery"
