...
BASIS_DEPS := basis/poly.h basis/spline.h stats.h special.h linalg/solver.h linalg/decomp.h linalg/factor.h linalg/mat.h
...
#        ad.h, dist/*.h, solver/*.h, filter/*.h (trend-cycle filters),
...
# model: core, plus nn/*.h, sd/*.h, varima/*.h and lp/*.h - model architectures with fitting APIs.
...
CORE_HEADERS := ad.h json.h special.h stats.h regression.h
CORE_SUBDIRS := linalg random dist dist/mv solver frame cluster inference basis filter
MODEL_SUBDIRS := nn sd varima lp
...
# float64 (STAT_CFLAGS): its first section regresses on 1, x, x^2, x^3 with x
# in the thousands, so the design's condition number is 8e13 and at float32 the
# SVD behind mat_cond does not converge on it. The example says so itself.
examples/basis_example: examples/basis_example.c $(BASIS_DEPS) frame/csv.h frame/frame.h
	$(CC) $(STAT_CFLAGS) -I. examples/basis_example.c $(LDLIBS) -o examples/basis_example
...
examples/singular_draws_example: examples/singular_draws_example.c linalg/solver.h linalg/decomp.h linalg/factor.h linalg/mat.h random/random.h
	$(CC) $(STAT_CFLAGS) -I. examples/singular_draws_example.c $(LDLIBS) -o examples/singular_draws_example
...
# float64 for the same reason as the tour above, and because it reports a
# likelihood ratio, a difference of two maxima.
examples/qvarma_fixed_parameters_example: examples/qvarma_fixed_parameters_example.c $(QVARMA_DEPS)
	$(CC) $(STAT_CFLAGS) -I. examples/qvarma_fixed_parameters_example.c $(LDLIBS) -o examples/qvarma_fixed_parameters_example
...
# dispatches to, from one caller and from one caller per hardware thread.
# -fopenmp because the concurrent column is half the answer - OpenBLAS's
# per-process buffer table is what makes concurrent small calls expensive, and
# without the flag the pragmas are discarded and every cell measures one
# caller. Built both ways, since the
...
libhpbench.so: tests/performance/bench_hp_filter.c filter/hp.h linalg/solver.h linalg/factor.h linalg/tensor.h linalg/mat.h frame/frame.h
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/performance/bench_hp_filter.c $(LDLIBS) -o libhpbench.so

librollingbench.so: tests/performance/bench_rolling_mean.c linalg/mat.h linalg/tensor.h frame/frame.h
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/performance/bench_rolling_mean.c $(LDLIBS) -o librollingbench.so

libcumsumbench.so: tests/performance/bench_cumsum.c linalg/mat.h linalg/tensor.h frame/frame.h
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/performance/bench_cumsum.c $(LDLIBS) -o libcumsumbench.so
...
tests/performance/cumsum_threshold: tests/performance/cumsum_threshold.c linalg/mat.h random/random.h
	$(CC) $(CFLAGS) -DMAT_DOUBLE -DMAT_CUMSUM_OMP_MIN_ROWS=1 -DMAT_CUMSUM_OMP_MIN_COLUMNS=1 -DMAT_CUMSUM_OMP_MAX=2147483647 tests/performance/cumsum_threshold.c $(LDLIBS) -o tests/performance/cumsum_threshold

tests/performance/rolling_mean_threshold: tests/performance/rolling_mean_threshold.c linalg/mat.h random/random.h
	$(CC) $(CFLAGS) -DMAT_DOUBLE -DMAT_ROLLING_OMP_MIN=1 -DMAT_ROLLING_OMP_MAX=2147483647 tests/performance/rolling_mean_threshold.c $(LDLIBS) -o tests/performance/rolling_mean_threshold

bench-rolling_mean_threshold: tests/performance/rolling_mean_threshold
	./tests/performance/rolling_mean_threshold

bench-cumsum_threshold: tests/performance/cumsum_threshold
...
tests/correctness/lstsq_rank_deficiency: tests/correctness/lstsq_rank_deficiency.c tests/check.h random/random.h linalg/solver.h linalg/decomp.h linalg/factor.h linalg/mat.h
	$(CC) $(CFLAGS) tests/correctness/lstsq_rank_deficiency.c $(LDLIBS) -o tests/correctness/lstsq_rank_deficiency

tests/correctness/mv_density_dispatch: tests/correctness/mv_density_dispatch.c tests/check.h random/random.h dist/mv/gauss.h dist/mv/student.h dist/mv/matgauss.h linalg/decomp.h linalg/factor.h linalg/mat.h special.h
	$(CC) $(CFLAGS) tests/correctness/mv_density_dispatch.c $(LDLIBS) -o tests/correctness/mv_density_dispatch
...
tests/correctness/rank_rule_consistency: tests/correctness/rank_rule_consistency.c tests/check.h random/random.h linalg/solver.h linalg/decomp.h linalg/factor.h linalg/mat.h
	$(CC) $(CFLAGS) tests/correctness/rank_rule_consistency.c $(LDLIBS) -o tests/correctness/rank_rule_consistency

tests/correctness/chol_singularity: tests/correctness/chol_singularity.c tests/check.h random/random.h linalg/decomp.h linalg/factor.h linalg/mat.h
	$(CC) $(CFLAGS) tests/correctness/chol_singularity.c $(LDLIBS) -o tests/correctness/chol_singularity
...
# The layer under mcs()'s entry points: the primitives the header calls
# public so a caller can run their own elimination loop, and the check
# that such a loop reproduces mcs() when its scratch is sized per round
# rather than once for the first one.
tests/correctness/mcs_primitives: tests/correctness/mcs_primitives.c inference/mcs.h tests/check.h stats.h random/random.h special.h frame/frame.h linalg/mat.h
	$(CC) $(CFLAGS) tests/correctness/mcs_primitives.c $(LDLIBS) -o tests/correctness/mcs_primitives

# Coverage and power over repeated samples, the property the procedure
# exists for and the one a comparison against a previous version's
# numbers cannot speak to. See the file header.
...
# The four basis/ suites. Built at the default precision rather than at
# float64, unlike the statistical binaries below: each one passes at both, and
# `make MAT_DOUBLE=1 test` is what covers the other half. Where a check needs a
# different tolerance at float32 it derives one from sizeof(mreal) rather than
# the suite being built one way.
tests/correctness/poly_correctness: tests/correctness/poly_correctness.c $(BASIS_DEPS) tests/check.h
	$(CC) $(CFLAGS) tests/correctness/poly_correctness.c $(LDLIBS) -o tests/correctness/poly_correctness

tests/correctness/spline_design_correctness: tests/correctness/spline_design_correctness.c $(BASIS_DEPS) tests/check.h
	$(CC) $(CFLAGS) tests/correctness/spline_design_correctness.c $(LDLIBS) -o tests/correctness/spline_design_correctness

tests/correctness/spline_basis_correctness: tests/correctness/spline_basis_correctness.c $(BASIS_DEPS) tests/check.h
	$(CC) $(CFLAGS) tests/correctness/spline_basis_correctness.c $(LDLIBS) -o tests/correctness/spline_basis_correctness

tests/correctness/spline_objects_correctness: tests/correctness/spline_objects_correctness.c $(BASIS_DEPS) tests/check.h
	$(CC) $(CFLAGS) tests/correctness/spline_objects_correctness.c $(LDLIBS) -o tests/correctness/spline_objects_correctness
...
# basis/poly.h and basis/spline.h against a live R and its splines package.
# Both sides compute the same deterministic functions, so this compares
# numbers element by element rather than distributions, which is what the lhs
# comparison next to it cannot do. R is a development-tier dependency (see
# README's Installation tiers), so this stays out of `test` and `check.sh`.
# Built at the precision MAT_DOUBLE selects rather than pinned to float64, so
# both halves of the claim can be checked: the R side reads sizeof(mreal) back
# out of the object and sets its tolerance from it.
libbasisagreement.so: tests/correctness/basis_r_agreement.c $(BASIS_DEPS)
	$(CC) $(CFLAGS) -shared -fPIC tests/correctness/basis_r_agreement.c $(LDLIBS) -o libbasisagreement.so

test-basis-r: libbasisagreement.so
	Rscript tests/correctness/basis_r_agreement.R

# basis/poly.h and basis/spline.h against R's stats and splines packages for
# speed, over the same entry points. Not part of bench.sh, which drives the
# Python comparison suites.
libbasisbench.so: tests/performance/bench_basis.c $(BASIS_DEPS)
	$(CC) $(STAT_CFLAGS) -shared -fPIC tests/performance/bench_basis.c $(LDLIBS) -o libbasisbench.so

bench-basis: libbasisbench.so
	Rscript tests/performance/bench_basis.R
...
bench-lhs: liblhs.so
	Rscript tests/performance/bench_lhs.R
...
# Checks mat_cumsum, tensor_cumsum and df_cumsum against numpy and polars in
# both precisions. Needs an interpreter with both: make test-cumsum-python PYTHON=...
CUMSUM_DEPS := tests/correctness/cumsum_reference_agreement.c linalg/mat.h linalg/tensor.h frame/frame.h
libcumsum_f64.so: $(CUMSUM_DEPS)
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/correctness/cumsum_reference_agreement.c $(LDLIBS) -o libcumsum_f64.so
libcumsum_f32.so: $(CUMSUM_DEPS)
	$(CC) $(filter-out -DMAT_DOUBLE,$(CFLAGS)) -shared -fPIC tests/correctness/cumsum_reference_agreement.c $(LDLIBS) -o libcumsum_f32.so
test-cumsum-python: libcumsum_f64.so libcumsum_f32.so
	$(or $(PYTHON),python3) tests/correctness/cumsum_reference_agreement.py

tests/correctness/hp_filter_correctness: tests/correctness/hp_filter_correctness.c filter/hp.h tests/check.h linalg/solver.h linalg/factor.h linalg/tensor.h linalg/mat.h frame/frame.h random/random.h
	$(CC) $(CFLAGS) tests/correctness/hp_filter_correctness.c $(LDLIBS) -o tests/correctness/hp_filter_correctness

tests/correctness/hp_filter_recovery: tests/correctness/hp_filter_recovery.c filter/hp.h tests/check.h linalg/solver.h linalg/factor.h linalg/tensor.h linalg/mat.h frame/frame.h random/random.h
	$(CC) $(STAT_CFLAGS) tests/correctness/hp_filter_recovery.c $(LDLIBS) -o tests/correctness/hp_filter_recovery

tests/correctness/rolling_mean_correctness: tests/correctness/rolling_mean_correctness.c tests/check.h linalg/mat.h linalg/tensor.h frame/frame.h random/random.h
	$(CC) $(CFLAGS) tests/correctness/rolling_mean_correctness.c $(LDLIBS) -o tests/correctness/rolling_mean_correctness

# Checks mat_rolling_mean, tensor_rolling_mean and df_rolling_mean against numpy
# and polars in both precisions: make test-rolling-mean-python PYTHON=...
ROLLING_DEPS := tests/correctness/rolling_mean_reference_agreement.c linalg/mat.h linalg/tensor.h frame/frame.h
librolling_f64.so: $(ROLLING_DEPS)
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/correctness/rolling_mean_reference_agreement.c $(LDLIBS) -o librolling_f64.so
librolling_f32.so: $(ROLLING_DEPS)
	$(CC) $(filter-out -DMAT_DOUBLE,$(CFLAGS)) -shared -fPIC tests/correctness/rolling_mean_reference_agreement.c $(LDLIBS) -o librolling_f32.so
# Checks filter/hp.h against statsmodels and lpirfs' algorithm in both
# precisions: make test-hp-filter-python PYTHON=...
HP_DEPS := tests/correctness/hp_filter_reference_agreement.c filter/hp.h linalg/solver.h linalg/factor.h linalg/tensor.h linalg/mat.h frame/frame.h
libhp_f64.so: $(HP_DEPS)
	$(CC) $(CFLAGS) -DMAT_DOUBLE -shared -fPIC tests/correctness/hp_filter_reference_agreement.c $(LDLIBS) -o libhp_f64.so
libhp_f32.so: $(HP_DEPS)
	$(CC) $(filter-out -DMAT_DOUBLE,$(CFLAGS)) -shared -fPIC tests/correctness/hp_filter_reference_agreement.c $(LDLIBS) -o libhp_f32.so
...
liblp_f32.so: $(LP_DEPS)
	$(CC) $(filter-out -DMAT_DOUBLE,$(CFLAGS)) -shared -fPIC tests/correctness/lp_reference_agreement.c $(LDLIBS) -o liblp_f32.so
...
tests/correctness/cumsum_correctness: tests/correctness/cumsum_correctness.c tests/check.h linalg/mat.h linalg/tensor.h frame/frame.h random/random.h
	$(CC) $(CFLAGS) tests/correctness/cumsum_correctness.c $(LDLIBS) -o tests/correctness/cumsum_correctness
...
tests/correctness/var_correctness: tests/correctness/var_correctness.c $(VAR_DEPS) tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/var_correctness.c $(LDLIBS) -o tests/correctness/var_correctness

tests/correctness/var_recovery: tests/correctness/var_recovery.c $(VAR_DEPS) tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/var_recovery.c $(LDLIBS) -o tests/correctness/var_recovery

tests/correctness/lp_correctness: tests/correctness/lp_correctness.c $(LP_MODEL_DEPS) tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/lp_correctness.c $(LDLIBS) -o tests/correctness/lp_correctness

tests/correctness/lp_recovery: tests/correctness/lp_recovery.c $(LP_MODEL_DEPS) tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/lp_recovery.c $(LDLIBS) -o tests/correctness/lp_recovery

tests/correctness/qvarma_fixed_parameter_fit: tests/correctness/qvarma_fixed_parameter_fit.c $(QVARMA_DEPS) special.h tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/qvarma_fixed_parameter_fit.c $(LDLIBS) -o tests/correctness/qvarma_fixed_parameter_fit
...
FRAME_TO_MODEL_DEPS := frame/csv.h frame/frame.h nn/mlp.h solver/adam.h \
                       solver/optimizer.h inference/mcs.h $(COINTEGRATION_DEPS) $(QVARMA_DEPS)
...
ALL_HEADERS := $(CORE_HEADERS) $(wildcard linalg/*.h random/*.h dist/*.h dist/mv/*.h solver/*.h frame/*.h cluster/*.h inference/*.h basis/*.h filter/*.h nn/*.h sd/*.h varima/*.h lp/*.h)
...
# Built at float64 like every other statistical binary here: it runs a
# least-squares fit on a design matrix built from levels data, and a model
# confidence set on the residuals of one.
tests/integration/basis_to_regression: tests/integration/basis_to_regression.c $(BASIS_DEPS) frame/csv.h frame/frame.h json.h inference/mcs.h tests/check.h
	$(CC) $(STAT_CFLAGS) -I. tests/integration/basis_to_regression.c $(LDLIBS) -o tests/integration/basis_to_regression
...
INTEGRATION_TESTS := tests/integration/frame_to_model tests/integration/frame_to_tensor \
                     ...
                     tests/integration/basis_to_regression
...
# A candidate rewrite of inference/mcs.h against the shipped header, on
# agreement first and on time and allocated bytes second. The occasion is
# a candidate that spends less memory; the harness is not specific to it.
#
#   make bench-mcs_candidates MCS_CANDIDATE=inference/mcs_lowmem.h
#   STRESS=1 make bench-mcs_candidates MCS_CANDIDATE=...   (adds the two widest cases)
#
# The candidate has to sit in inference/ beside the header it would
# replace: it is a copy of that file and inherits its relative includes,
# so ../frame/frame.h has to resolve from where it is compiled.
#
# With no MCS_CANDIDATE both arms are the shipped header, which is the
# harness checking itself: the two must then agree bit for bit and time
# within noise. inference/mcs.h and any candidate for it are static
# inline throughout, so the same source compiled against each produces
# two objects that share no symbol and link into one binary - which is
# what lets the two arms alternate inside a single process rather than
# being timed in two runs an hour apart. The recipe rebuilds both arms
# every time, since MCS_CANDIDATE is a command-line variable and make
...
test-mcs-candidate:
	...
	$(CC) $(CFLAGS) -I. -DMCS_HEADER='"$(MCS_CANDIDATE)"' tests/correctness/mcs_size_and_power.c $(LDLIBS) -o tests/correctness/mcs_size_and_power_candidate
...
bench-mcs_candidates: tests/performance/mcs_candidates.c tests/performance/mcs_arm.c $(MCS_ARM_DEPS) $(MCS_CANDIDATE)
	$(CC) $(CFLAGS) -I. -DMCS_ARM_HEADER='"inference/mcs.h"' -DMCS_ARM_ENTRY=mcs_arm_current -c tests/performance/mcs_arm.c -o tests/performance/mcs_arm_current.o
	$(CC) $(CFLAGS) -I. -DMCS_ARM_HEADER='"$(MCS_CANDIDATE)"' -DMCS_ARM_ENTRY=mcs_arm_candidate -c tests/performance/mcs_arm.c -o tests/performance/mcs_arm_candidate.o
	$(CC) $(CFLAGS) -I. -DMCS_ARM_CURRENT_HEADER='"inference/mcs.h"' -DMCS_ARM_CANDIDATE_HEADER='"$(MCS_CANDIDATE)"' tests/performance/mcs_candidates.c tests/performance/mcs_arm_current.o tests/performance/mcs_arm_candidate.o $(LDLIBS) -o tests/performance/mcs_candidates
	rm -f tests/performance/mcs_arm_current.o tests/performance/mcs_arm_candidate.o
...
# failing is the signal that a kernel is not ready for production. They run
# at OpenBLAS's full thread count, per README's rule that performance is
# measured at full capacity; each report records the count it ran with.
...
install-model: install-core
	...
	@printf 'prefix=%s\nincludedir=$${prefix}/include/et_al.\n\nName: et_al.-model\nDescription: ET_AL. model layer - model architectures with fitting APIs (nn/, sd/, varima/, lp/)\nVersion: %s\nRequires: et_al.-core\nCflags: -I$${includedir}\nLibs:\n' \
