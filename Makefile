...
# The fused filter against the traced one it replaced in the fit. float64: it
...
tests/correctness/qvarma_fused_agreement: tests/correctness/qvarma_fused_agreement.c $(QVARMA_DEPS) tests/check.h
	$(CC) $(STAT_CFLAGS) tests/correctness/qvarma_fused_agreement.c $(LDLIBS) -o tests/correctness/qvarma_fused_agreement
...
# The fused filter against the taped one it replaced in the fit, value and
# value+gradient, at one and four threads. -fopenmp because the four-thread
# column is the point: without the flag the pragmas are discarded and every
# cell measures one thread. Built both ways, since the fit runs at either
# precision. Run directly:
#   make bench-qvarma_fused_filter
tests/performance/qvarma_fused_filter: tests/performance/qvarma_fused_filter.c $(QVARMA_DEPS)
	$(CC) $(MODEL_CFLAGS) -fopenmp tests/performance/qvarma_fused_filter.c $(LDLIBS) -o tests/performance/qvarma_fused_filter

tests/performance/qvarma_fused_filter_float64: tests/performance/qvarma_fused_filter.c $(QVARMA_DEPS)
	$(CC) $(STAT_CFLAGS) -fopenmp tests/performance/qvarma_fused_filter.c $(LDLIBS) -o tests/performance/qvarma_fused_filter_float64

bench-qvarma_fused_filter: tests/performance/qvarma_fused_filter tests/performance/qvarma_fused_filter_float64
	./tests/performance/qvarma_fused_filter
	./tests/performance/qvarma_fused_filter_float64
